...
export default class Lwc_accountsTitleDropdown extends LightningElement {
    ...
    calculohora(){
        if(this.accountlastupdate != undefined){
            var fecha = this.accountlastupdate;
            var lfecha = fecha.split(" ");
            if(lfecha[1]){
                var ltiempo = lfecha[1].split(":");
                if(ltiempo[0] && ltiempo[1]) this.upToHour = ltiempo[0]+":"+ ltiempo[1];
            }
        }
    }
