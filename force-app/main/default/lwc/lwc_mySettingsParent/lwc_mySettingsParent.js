...
export default class Lwc_mySettingsParent extends LightningElement {
  ...
  setPicklistValues(response) {
    if (response != null) {
      ...
      responseParsed.TimeZoneList.forEach((element) =>
        timeZonesList.push(element.label)
      );
      responseParsed.LanguageList.forEach((element) =>
        languageList.push(element.label)
      );
      responseParsed.CurrencyList.forEach((element) =>
        CurrenciesList.push(element.label)
      );
      responseParsed.CurrencyPairsList.forEach((element) =>
        CurrenciesPairList.push(element.label)
      );
      responseParsed.NumberFormatList.forEach((element) =>
        numberFormatList.push(element.label)
      );
      responseParsed.DateFormatList.forEach((element) =>
        dateFormatList.push(element.label)
      );
