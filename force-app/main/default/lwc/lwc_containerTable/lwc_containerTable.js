...
export default class Lwc_containerTable extends LightningElement {
  ...
  getTradesHandler(jsonFilter) {
    ...
    searchFIlterTradesColumns({ parameters: jsonFilter })
      .then((result) => {
    ...
    return new Promise(
      function (resolve, reject) {
        getUserData()
              ...
              if (result.value) {
                if (result.value.userData) {
                  currentUser = JSON.parse(
                    JSON.stringify(result.value.userData)
          ...
          .catch((errors) => {
            console.log(
              "### lwc_paymentsLandingParent ### getCurrentUserData() ::: Catch Error: " +
                errors
            );
            reject({
              title: errorLoading,
  ...
  getFilterStatusValues(context) {
    getStatus({ context: context })
      ...
      .finally(() => {
        this.template
          .querySelector("c-lwc_fx_trades-landing-filters")
          .setFilters();
      })
      .catch((error) => {
        this.error = error;
      ...
      removeFile({
        ...
        .catch((errors) => {
          if (errors) {
            ...
            console.log(
              "### lwc_paymentsLandingParent ### removeFile(event, ID) ::: Catch Error: " +
                errors
            );
          }
        });
    ...
    return new Promise(function (resolve, reject) {
      if (documentId != null && documentId != "" && documentId != undefined) {
        window.location.href =
          "https://lwr-santanderonetrade.cs109.force.com/otfx/sfsites/c/sfc/servlet.shepherd/document/download/" +
          documentId;
        resolve(documentId);
      } else {
        reject({
  ...
  handleOrderValueChange(event) {
    ...
    this.columns.forEach(function (column) {
      ...
      if (column.key == columnId) {
        if (
          column.styleColumn ==
            "button-orderRight icon-arrowDown color:white" ||
          column.styleColumn ==
            "button-orderRight icon-arrowUp color:white button-orderOpacity"
        ) {
          column.styleColumn =
            "button-orderRight icon-arrowDown color:white button-orderOpacity";
