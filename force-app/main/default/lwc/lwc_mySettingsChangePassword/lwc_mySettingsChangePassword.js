...
export default class Lwc_mySettingsChangePassword extends LightningElement {
  ...
  cancelchangepassword() {
    ...
    const buttoncancelchangepassword = new CustomEvent(
      "buttoncancelchangepassword"
    );
    this.dispatchEvent(buttoncancelchangepassword);
  }
  handleSave() {
    var oldPassword = this.template.querySelector('[data-id="oldPassword"]')
      .value; //        find("oldPassword").get("v.value");
    var newPassword = this.template.querySelector('[data-id="newPassword"]')
      .value; //var newPassword = this.template.find("newPassword").get("v.value");
    var newPasswordConfirmed = this.template.querySelector(
      '[data-id="newPassword2"]'
    ).value; //var newPasswordConfirmed = this.template.find("newPassword2").get("v.value");
