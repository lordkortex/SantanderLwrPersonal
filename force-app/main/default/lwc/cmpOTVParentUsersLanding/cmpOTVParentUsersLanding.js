...
import lwc_parent_users_landing       from '@salesforce/resourceUrl/lwc_parent_users_landing';
...
export default class CmpOTVParentUsersLanding extends LightningElement {
    ...
    connectedCallback(){
        Promise.all([
            ...
            loadStyle(this,lwc_parent_users_landing)
        ])
        getCheckBox().then((result)=>{
            console.log('Entra checkbox: ' + result);
    ...
    get checkWelcomeModal(){
        ...
        if(!this.checkboxWelcomePack && !this.loading && !this.isBackfront){
            return false;
        }else{
            return true;
        }
