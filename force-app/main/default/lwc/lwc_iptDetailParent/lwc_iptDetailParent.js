...
export default class lwc_iptDetailParent extends LightningElement{
    ...
    getDataEvent() {
        //CÃ³digo comentado: this.totalElapsedTime = event.getParam('elapsed');
    }

    getDataObject() {
        try{
            var params = ''; //this.sendobject; //GAA y este  atributo de dÃ³nde sale???
            ...
            this.showerror = true;
            this.dispatchErrorLoadEvent();
            console.log('### lwc_iptDetailParent ### getDataObject() ::: Catch Error: ' + e);
        }
    }
       ...
       try {
            ...
            if(isIngested){
                if(this.comesfromtracker == true) {
                    ...
                    UETRSearchResult.paymentDetail.paymentId                                        === undefined ? 'Not found' : this.iobject.uetr = UETRSearchResult.paymentDetail.paymentId;              
                    UETRSearchResult.paymentDetail.transactionStatus.reason                         === undefined ? 'Not found' : this.iobject.reason = UETRSearchResult.paymentDetail.transactionStatus.reason;              
                    UETRSearchResult.paymentDetail.transactionStatus.status                         === undefined ? 'Not found' : this.iobject.status = UETRSearchResult.paymentDetail.transactionStatus.status;              
                    UETRSearchResult.paymentDetail.originatorData.originatorAccount.accountId       === undefined ? 'Not found' : this.iobject.originAccountNumber = UETRSearchResult.paymentDetail.originatorData.originatorAccount.accountId;              
                    UETRSearchResult.paymentDetail.originatorData.originatorName                    === undefined ? 'Not found' : this.iobject.originAccountName = UETRSearchResult.paymentDetail.originatorData.originatorName;              
                    UETRSearchResult.paymentDetail.originatorAgent.agentName                        === undefined ? 'Not found' : this.iobject.originAccountBank = UETRSearchResult.paymentDetail.originatorAgent.agentName;              
                    UETRSearchResult.paymentDetail.originatorAgent.agentCode                        === undefined ? 'Not found' : this.iobject.originAccountBic = UETRSearchResult.paymentDetail.originatorAgent.agentCode;              
                    UETRSearchResult.paymentDetail.beneficiaryData.creditorCreditAccount.accountId  === undefined ? 'Not found' : this.iobject.beneficiaryAccountNumber = UETRSearchResult.paymentDetail.beneficiaryData.creditorCreditAccount.accountId;
                    UETRSearchResult.paymentDetail.beneficiaryData.beneficiaryName                  === undefined ? 'Not found' : this.iobject.beneficiaryAccountName = UETRSearchResult.paymentDetail.beneficiaryData.beneficiaryName;              
                    UETRSearchResult.paymentDetail.beneficiaryData.creditorAgent.agentName          === undefined ? 'Not found' : this.iobject.beneficiaryAccountBank = UETRSearchResult.paymentDetail.beneficiaryData.creditorAgent.agentName;              
                    UETRSearchResult.paymentDetail.beneficiaryData.creditorAgent.agentCode          === undefined ? 'Not found' : this.iobject.beneficiaryAccountBic = UETRSearchResult.paymentDetail.beneficiaryData.creditorAgent.agentCode;                                
                    UETRSearchResult.paymentDetail.paymentAmount.tcurrency                          === undefined ? 'Not found' : this.iobject.currencyAux = UETRSearchResult.paymentDetail.paymentAmount.tcurrency;              
                    UETRSearchResult.paymentDetail.valueDate                                        === undefined ? 'Not found' : this.iobject.valueDate = UETRSearchResult.paymentDetail.valueDate;              
                    UETRSearchResult.paymentDetail.paymentAmount.amount                             === undefined ? 'Not found' : this.iobject.amount = UETRSearchResult.paymentDetail.paymentAmount.amount;              
                    UETRSearchResult.paymentDetail.beneficiaryData.creditorAgent.agentLocation      === undefined ? 'Not found' : this.iobject.beneficiaryCity = UETRSearchResult.paymentDetail.beneficiaryData.creditorAgent.agentLocation;              
                    UETRSearchResult.paymentDetail.beneficiaryData.creditorAgent.agentCountry       === undefined ? 'Not found' : this.iobject.beneficiaryCountry = UETRSearchResult.paymentDetail.beneficiaryData.creditorAgent.agentCountry;
                
                } else{
                    
                    UETRSearchResult.paymentId                                          === undefined ? 'Not found' : this.iobject.uetr = UETRSearchResult.paymentId;              
                    UETRSearchResult.transactionStatus.reason                           === undefined ? 'Not found' : this.iobject.reason = UETRSearchResult.transactionStatus.reason;
                    UETRSearchResult.transactionStatus.status                           === undefined ? 'Not found' : this.iobject.status = UETRSearchResult.transactionStatus.status;              
                    UETRSearchResult.debtorAgent.agentName                              === undefined ? 'Not found' : this.iobject.originAccountBank = UETRSearchResult.debtorAgent.agentName;              
                    UETRSearchResult.debtorAgent.agentCode                              === undefined ? 'Not found' : this.iobject.originAccountBic = UETRSearchResult.debtorAgent.agentCode;              
                    UETRSearchResult.creditorAgent.agentName                            === undefined ? 'Not found' : this.iobject.beneficiaryAccountBank = UETRSearchResult.creditorAgent.agentName;              
   ...
   getURLParams() {
                ...
                this.decrypt(sURLVariablesMain[1]).then((results) => {
                    ...
                    for ( var i = 0; i < sURLVariables.length; i++ ) {
                        ...
                        if (sParameterName[0] === 'c__filters') { 
                            sParameterName[1] === undefined ? 'Not found' : this.filters = sParameterName[1];              
                        }                          
                    }
    ...
    getShowFee(i) {
        try{
            ...
            .catch(error => {
                console.log('### lwc_iptDetailParent ### getShowFee(i) ::: Try error: ' + error);
                this.showerror = true;
                this.dispatchErrorLoadEvent();
            });  
        }catch(e){
            this.showerror = true;
            this.dispatchErrorLoadEvent();
            console.log('### lwc_iptDetailParent ### getShowFee(i) ::: Catch error: ' + e);
        }
    }
    ...
    getBicListFull(){
        try{
            ...
            .catch(error => {
                console.log('### lwc_iptDetailParent ### getBicListFull() ::: Try error: ' + error);
            })         
        }
        catch(e){
            console.log('### lwc_iptDetailParent ### getBicListFull() ::: Catch error: ' + e);
        }
    }
    ...
    decrypt(data){
        try {
            ...
            .catch(error => {
                console.log('### lwc_iptDetailParent ### decrypt() ::: Try Error: ' + error);
                reject(error);
                this.showerror = true;
                this.dispatchErrorLoadEvent();
            }); 
            ...
            console.error('### lwc_iptDetailParent ### decrypt() ::: Catch Error: ' + e);
        }
    }
   ...
   getBICS() {
    try {
        ...
        .catch(error => {
            console.log('### lwc_iptDetailParent ### getBICS() ::: Try Error: ' + error);
        });

    } catch(e) {
        console.error('### lwc_iptDetailParent ### getBICS() ::: Catch error: ' + e);
    }
    }
    ...
    getDataSteps() {
        ...
        try {
            ...
            this.showerror = true;
            this.dispatchErrorLoadEvent();
            console.error('### lwc_iptDetailParent ### getDataStepsCatch() ::: Error: ' + e);
        }
    }
    ...
    getReason() {
        try{
            ...
            if(this.iobject.status == 'RJCT'){
                ...
                .catch(error => {
                    console.log('### lwc_iptDetailParent ### getReasonTry error: ' + error);
                });
            }
        }catch(e){
