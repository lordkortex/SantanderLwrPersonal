...
export default class Lwc_cn_calendar extends LightningElement {
  ...
  get getCalendarClass() {
    return this.hasseparation
      ? "slds-form slds-form_compound calendar calendar--double flexedCalendars"
      : "slds-form slds-form_compound calendar calendar--double";
  }

  get getCalendarClass2() {
    return this.hasseparation
      ? "slds-form-element calendarSeparated"
      : "slds-form-element calendar";
  }
  ...
  handleBlurFrom() {
    ...
    if (!this.check2.valid) {
      this.helpTextFrom = "";
      this.fromCMP.setCustomValidity(
        this.label.allowedDateFormat + " " + shortFormat
      );
    } else {
      if (!this._dates) {
        this._dates = [];
  ...
  clearData() {
    this._dates = [];
    if (this.template.querySelector("[data-id='dateFrom']")) {
      this.template.querySelector("[data-id='dateFrom']").value = "";
    }
    if (this.template.querySelector("[data-id='dateTo']")) {
      this.template.querySelector("[data-id='dateTo']").value = "";
    }
  }
}
