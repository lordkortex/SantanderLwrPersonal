...
export default class Lwc_fx_tradesLandingFiltersModal extends LightningElement {
  ...
  get showMiniLabelORsearchSourceNE() {
    ...
    return (
      this.showsourceaccountminilabel ||
      (this.searchedsourceaccount != undefined &&
        this.searchedsourceaccount != null &&
  ...
  get isEmptyAccountSuggestions2() {
    ...
    var ret = "";
    if (
      this.accountsuggestions == undefined ||
      this.accountsuggestions == null ||
  ...
  get showFromMiniLabelORformattedValueFromNE() {
    ...
    return (
      this.showfromminilabel ||
      (this.formattedvaluefrom != undefined &&
        this.formattedvaluefrom != null &&
  ...
  get showToMiniLabelORformattedValueToNE() {
    ...
    return (
      this.showtominilabel ||
      (this.formattedvalueto != undefined &&
        this.formattedvalueto != null &&
  ...
  get showCRMiniLabelORclientReferenceNE() {
    ...
    return (
      this.showclientreferenceminilabel ||
  ...
  get datesPlaceholdersZeroNEvdFrom() {
    ...
    return (
      this.datesplaceholders &&
      this.datesplaceholders[0] != this.label.valueDateFrom
    );
  }
  ...
  applyFilterModal() {
    ...
    if (
      errorDate != true &&
      errorToDateFormat != true &&
      errorFromDateFormat != true &&
      errorAmount != true &&
      errorAccount == ""
    ) {
  ...
  setClientReference(event) {
    ...
    if (!isDisabled) {
      ...
      if (event.target.value != undefined) {
        ...
        let regExp = new RegExp("^[0-9a-zA-Zs]{0,16}$");
  ...
  handleInputAmount(event) {
    ...
    if (inputValue != undefined && inputValue != null) {
      ...
      let valueWithDecimal = valueWithoutThousand.replace(
        decimalSeparator,
        "."
      );

      if (
        parseFloat(valueWithDecimal) != undefined &&
        Number.isNaN(parseFloat(valueWithDecimal)) == false
      ) {
        if (event.currentTarget.id == "fromAmount") {
  ...
  handleFocusAmount(event) {
    ...
    if (inputId != undefined && inputId != null) {
      if (inputId == "fromAmount") {
        if (
          this.userinputfrom != undefined &&
          this.userinputfrom != null &&
          this.userinputfrom != ""
        ) {
  ...
  searchAccounts(searchedString) {
    ...
    if (
      ...
      for (
        let i = 0;
        i < accountList.length && accountSuggestions.length < 5;
        i++
      ) {
  ...
  setFilter() {
    ...
    if (selectedBox != undefined && selectedBox != null) {
      if (
        selectedBox.parsedStatusDescription == this.label.PAY_Status_PendingOne
      ) {
        var aux = "";
        if (
          selectedBox.statusName != undefined &&
          selectedBox.statusName != null
        ) {
        ...
        selectedBox.parsedStatusDescription == this.label.PAY_Status_PendingTwo
      ) {
        var aux = "";
        if (
          selectedBox.statusName != undefined &&
        ...
        selectedBox.parsedStatusDescription == this.label.PAY_Status_InReviewOne
      ) {
        var obj = statusList.find(
          (obj) => obj.value == "chk_" + selectedBox.statusName
        );
        selectedStatus.push(obj.value);
        this.selectedstatuses = selectedStatus;
      } else if (
        selectedBox.parsedStatusDescription ==
        this.label.PAY_Status_ScheduledOne
      ) {
        var obj = statusList.find(
          (obj) => obj.value == "chk_" + selectedBox.statusName
        );
        ...
        var nextWeek =
          toDate.getFullYear() + "-" + toMonth + "-" + toDate.getDate();
        d.push(nextWeek);

        this.dates = d;
      } else if (
        selectedBox.parsedStatusDescription ==
        this.label.PAY_Status_CompletedOne
      ) {
        var obj = statusList.find(
          (obj) => obj.value == "chk_" + selectedBox.statusName
        );
        ...
        selectedBox.parsedStatusDescription == this.label.PAY_Status_RejectedOne
      ) {
        var obj = statusList.find(
          (obj) => obj.value == "chk_" + selectedBox.statusName
        );
      ...
      this.template
        .querySelector('[data-id="statusDropdownModal"]')
        .setSelectedValues(this.selectedstatuses); //setSelectedValues metodo @api
    } else {
      var selectedCurrencies = this.selectedcurrencies;
      if (selectedCurrencies != undefined && selectedCurrencies != null) {
        this.template
          .querySelector('[data-id="currencyDropdownModal"]')
          .setSelectedValues(this.selectedcurrencies);
      }
      var selectedStatuses = this.selectedstatuses;
      if (selectedStatuses != undefined && selectedStatuses != null) {
        this.template
          .querySelector('[data-id="statusDropdownModal"]')
          .setSelectedValues(this.selectedstatuses);
      }
      ...
      if (selectedCountry != undefined && selectedCountry != null) {
        ...
        this.template
          .querySelector('[data-id="countryDropdownModal"]')
      ...
      if (selectedMethod != undefined && selectedMethod != null) {
        ...
        this.template
          .querySelector('[data-id="methodDropdownModal"]')
          .setSelectedValues(items);
      }
  ...
  showToast() {
    ...
    if (errorToast != undefined && errorToast != null) {
      ...
      errorToast.showToast(
        false,
        false,
        this.label.B2B_Error_Problem_Loading,
        this.label.B2B_Error_Check_Connection,
        "Error",
        "Warning",
        "Warning",
        false
  ...
  handleChangeDatesInternal() {
    ...
    if (placeholders[0] == fromPlaceholder) {
      ...
      if (this.checkDateFormat(placeholders[0])) {
        //has valid format
        this.fromdateformat = false;
