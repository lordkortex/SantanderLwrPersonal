...
export default class Lwc_fx_tradesLandingBoxes extends LightningElement {
  ...
  connectedCallback() {
    ...
    var paymentstatusboxesAux = JSON.parse(
      JSON.stringify(this.paymentstatusboxes)
    );
    console.log("Payments status boxes: " + paymentstatusboxesAux);
    Object.keys(paymentstatusboxesAux).forEach((key) => {
      paymentstatusboxesAux[key].class1 =
        paymentstatusboxesAux[key].statusName ==
        this.selectedpaymentstatusbox.statusName
          ? "slds-col slds-button slds-button-active"
          : "slds-col slds-button";
  ...
  boxesFilters(event) {
    ...
    if (clickedBox == "Pending") {
      this.filters = {
        filtersRequired: 1,
        statusSelected: ["Pending To Be Confirmed"]
      };
