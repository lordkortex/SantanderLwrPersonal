...
export default class Lwc_paymentsLandingDownloadModal extends LightningElement {
  ...
  connectedCallback() {
    ...
    for (const val in this.values) {
      ...
      var cls =
        this.values[val] == this._selectedvalue
  ...
  get contat() {
    if (this.isotfxcommunity == false) {
      return (
        this.Label.PAY_DownloadBasicMsgOne +
        " " +
        this.Label.PAY_DownloadDescriptionOne
      );
    } else {
      return (
        this.Label.OTFX_DownloadBasicMsgOne +
        " " +
        this.Label.OTFX_DownloadDescriptionOne
      );
    }
  }
  ...
  get format1() {
    if (this.isotfxcommunity == false) {
      return this.formatLabel(
        this.Label.PAY_DownloadDescriptionTwo,
        this.listlength
      );
    } else {
      return this.formatLabel(
        this.Label.OTFX_DownloadDescriptionTwo,
        this.listlength
      );
    }
  }
  ...
  get format2() {
    if (this.isotfxcommunity == false) {
      var formatval = this.formatLabel(
        this.Label.PAY_DownloadBasicMsgTwo,
        this.maxnum
      );
      return formatval + " " + this.Label.PAY_DownloadDescriptionThree;
    } else {
      var formatval = this.formatLabel(
        this.Label.OTFX_DownloadBasicMsgTwo,
        this.maxnum
      );
  ...
  get getclass2() {
    return this.item == v.selectedvalue
      ? "slds-dropdown__item slds-is-selected"
      : "slds-dropdown__item";
  }
  ...
  handleApply() {
    ...
    if (selectedFormat) {
      ...
      this.iserror = false;
      const selectedEvent = new CustomEvent("applydownload", {
