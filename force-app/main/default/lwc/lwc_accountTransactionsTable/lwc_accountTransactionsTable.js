...
export default class Lwc_account_TransactionsTable extends LightningElement {
	...
	sortBy(event) {
        try {
            ...
            if(event.target.dataset.id){
                ...
                var sortItem = "v.sort" + event.target.dataset.id;
    ...
    navigateToExtractDetail(event) {
        ...
        var transactionRow = this.transactionresults[event.currentTarget.id];
    ...
    get iterationTransactionResults(){
        ...
        if(this.transactionresults){
            ...
            var items = new Array();
            var aux = JSON.parse(JSON.stringify(this.transactionresults.slice(this.start,this.end + 1)));
