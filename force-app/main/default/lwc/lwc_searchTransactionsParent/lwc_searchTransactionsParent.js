...
export default class Lwc_searchTransactionsParent extends LightningElement {
    ...
    get toastErrorLimit(){
        return this.label.limitTransactionSearch + ' ' + this.label.limitTransactionSearchNumber;
    }
    ...
    get isTransactionsResultsNull(){
        return this.wholeTransactionResults == null || this.transactionResults.length == 0;
    }
	...
	filterTableData(event){
		...
		if(event){
			...
			if(filters.length > 0 && !(filters[0].name == this.label.amount && (JSON.stringify(filters[0].value) == JSON.stringify({}) || (filters[0].value.from == undefined && filters[0].value.to == undefined) ))){
	...
	updateDropdownFilters (event) {
        ...
        if((event.detail.name == "onOptionSelection" && event.detail.source == "clearAll") || event.detail.source == "clearAll" || event.detail.name == "clearAllFilters"){ // MIRAR TRAS PONER ESCUCHADOR
			...
			var accountCodeToInfoMap = JSON.parse(JSON.stringify(this.accountCodeToInfo));
				...
				if(filterName != this.label.Category){
					...
					for(var oFilters in defaultFilters){
						...
						for(var key in filterKeys){
							...
							} else if(defaultFilters[oFilters].type == "text" || defaultFilters[oFilters].name == this.label.Category){
        ...
        if(response != "") {
            ...
            for(var i = 0; i < params.length ; i++) {
                ...
                if(sParameterName[0] == "c__filters") {
                    ...
                    sParameterName[1] === undefined  || sParameterName[1] == "undefined" || sParameterName[1] == "" ? this.accountCodesToSearch = [] : this.accountCodesToSearch = JSON.parse(sParameterName[1]);
                }
            }
    ...
    setUserDateNumberFormat (response){
        ...
        if(response != null){
            ...
            if(this.endOfDay == false){
                ...
                if(storageBalanceEOD != 'null' && storageBalanceEOD != undefined && balanceTimestampGP != 'null' && balanceTimestampGP != undefined && (new Date() - new Date(Date.parse(balanceTimestampGP))) < parseInt(this.refreshBalanceCollout)*60000 ){
            ...
            .then( res => {
                ...
                if(result != 'null' && result != undefined && result != null){
                    ...
                    if(this.endOfDay == false){
                        ...
                        if(storageBalance == 'null' || storageBalance == undefined ||  ( balanceTimestampGP != 'null' && balanceTimestampGP != undefined && (parseInt(this.label.refreshBalanceCollout)*60000 < new Date() - new Date(Date.parse(balanceTimestampGP))) )   ){
                        ...
                        var storageBalanceEOD = window.localStorage.getItem(userId + '_' + 'balanceEODGP');
                        var balanceTimestampGP = window.localStorage.getItem(userId + '_' + 'balanceEODTimestampGP');
                        if(storageBalanceEOD == 'null' || storageBalanceEOD == undefined ||  ( balanceTimestampGP != 'null' && balanceTimestampGP != undefined && (parseInt(this.label.refreshBalanceCollout)*60000 < new Date() - new Date(Date.parse(balanceTimestampGP))) )   ){
        ...
        for(var key in filters){
            if(filters[key].type == "checkbox"){
                ...
                if (filters[key].value != undefined && 
                    ((filters[key].value.from != undefined && filters[key].value.from != "") 
    ...
    handleSuccessCallback(event){
        ...
        else if(event.detail.callercomponent == "sendDataServiceRequestWithFilters"){
