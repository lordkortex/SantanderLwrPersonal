...
export default class Lwc_iam_accountTransactionsParent extends LightningElement {
    ...
    get islistaObtWholeTransaccionesCero(){
        if(this.comunidadCashNexus && this.wholeTransactionResults) return this.wholeTransactionResults.listaObtTransacciones[0];
    }
    ...
    get tableContent(){
        ...
        if(!this.comunidadCashNexus){
            ...
            return (JSON.stringify(this.transactionResults).length < 50 ) && (this.wholeTransactionResults == null || JSON.stringify(this.wholeTransactionResults).length < 10);
    }
        ...
        .then(value => {
            ...
            for ( var i = 0; i < sURLVariables.length; i++ ) {
                ...
                if (sParameterName[0] === 'c__lastUpdate') { 
                    sParameterName[1] === undefined ? 'Not found' : sParameterName[1] === 'true' ? this.lastUpdate = true : this.lastUpdate = false;
                }
                ...
                if (sParameterName[0] === 'c__selectedFilters' && !this.comunidadCashNexus) { 
            ...
            if(!this.comunidadCashNexus){
                ...
                if(this.selectedFilters.length == 0){
                    ...
                    bodyParams = this.setBodyParamsWithFilters(accountDetails, 1, this.selectedFilters);
                }
            ...
            //this.loading = false;
                ...
                //Get params    
                    ...
                    if(isEndOfDay){
                        this.template.querySelector("c-lwc_service-component").retrieveFromCache({callercomponent:'lwc_iam_accountTransactionsParent',key:'balanceEODGP'});
                    }else{
                        this.template.querySelector("c-lwc_service-component").retrieveFromCache({callercomponent:'lwc_iam_accountTransactionsParent',key:'balanceGP'});
                    }
                }else{
                    this.sendDataServiceRequestWithFilters(this.selectedFilters,isEndOfDay);
                }
            }            
    ...
    successcallback(event){	
        ...
        if(event.detail.callercomponent === 'lwc_iam_accountTransactionsParent_Number'){
            this.setUserDateNumberFormat(event.detail.value);
        }
        if(event.detail.callercomponent === 'lwc_iam_accountTransactionsParent_getTransactions'){
            this.setTableData(event.detail.value);
        }
        if(event.detail.callercomponent === 'lwc_iam_accountTransactionsParent_getTransCat'){
            this.getCategoryList(event.detail.value);
        }
        if(event.detail.callercomponent === 'lwc_iam_accountTransactionsParent_getTrans'){
            this.setTableData(event.detail.value);
        }        
    ...
    setUserNumberFormat(response){
		if(response != null){
			...
			if(parsedResponse != null && parsedResponse.mapUserFormats != null){
				this.userPreferredNumberFormat = parsedResponse.mapUserFormats.numberFormat;
			}
		}
    ...
    setUserDateNumberFormat(response){
		if(response != null){
			...
			//AM - 14/10/2020 - Fix Recieve format number from Cache.
			if(this.userPreferredNumberFormat == undefined || this.userPreferredNumberFormat == null){
            ...
            this.template.querySelector("c-lwc_service-component").onCallApex({callercomponent:'lwc_iam_accountTransactionsParent_getTransactions',controllermethod:'getTransactionsByAccount',actionparameters:params});
		}
	}
    ...
    setTableData(response){
		if(!this.comunidadCashNexus){
            ...
            if(response != '' && response != null   && response != undefined && Object.keys(response).length > 0){
			    ...
			    //COUNTRY -> BR,KY,LU
                if(this.accountDetails.country == "BR" || this.accountDetails.country == "KY" || this.accountDetails.country == "LU"){
                ...
                if(this.isInitialLoad && !this._isAdvancedSearch && response.balances.transactions.listaObtTransacciones.length != 0){
                    ...
                    if(response.balances.transactions.listaObtTransacciones[0][0] !=  undefined || response.balances.transactions.listaObtTransacciones[0].length > 0){
                        this.accountDetails.codigoBic = response.balances.transactions.listaObtTransacciones[0][0].obtTransacBusqueda.codigoBIC; 
                    }else {                    
            ...
            if(this.sourcePage != 'accountTransactions'){
                if(!this._isAdvancedSearch && (response != '' && response != null   && response != undefined)){
                    this.setFiltersData();
                }else{
            ...
            // If the previous page was Account Transactions Detail, it means the filters must be applied as they come from the URL
                ...
                if(JSON.stringify(this.selectedFilters) == []){
                    this.template.querySelector("c-lwc_service-component").onCallApex({callercomponent:'lwc_iam_accountTransactionsParent_getTransCat',controllermethod:'getTransactionCategories',actionparameters:{}});
                }
            }
        }
		this.loading = false;
    ...
    getLatestTimestampFromTransactions(){
		...
		if(transactions != undefined){
			...
			} else if(!this.comunidadCashNexus && transactions.listaObtTransacciones != undefined && transactions.listaObtTransacciones.length > 0 && transactions.listaObtTransacciones[0].obtTransacBusqueda != null){
    ...
    sendDataServiceRequestWithFilters(filters,isEndOfDay){    
        ...
        if(!this.comunidadCashNexus){
            ...
            if(this.selectedTimeframe != this.label.selectOne){
                ...
                params.obtTransacBusqueda.entrada.cacheKey.fechaHasta = toDate.toISOString();
                let fromDate = this.getBookDateFromTimePeriod(this.selectedTimeframe, toDate);
                params.obtTransacBusqueda.entrada.cacheKey.fechaDesde = fromDate;	  
                if(this.dates.length != 0){
                    this.changedTimeFrame = false;
                }	
    ...
    calculateNumberActiveFilters() {
		...
		for(var key in filters){
			if(filters[key].type == 'checkbox'){
				filterCount = filters[key].value.filter(option => option.checked).length > 0 ? filterCount+1 : filterCount;
                //filterCount++;
    ...
    filterByTimePeriod(event){
        ...
        if(timePeriod != this.label.selectOne){
            ...
            this.dates = [];
            if(!this.comunidadCashNexus) this.sendDataServiceRequestWithFilters(selectedFilters);			
            else{
                var isEndOfDay = !this.lastUpdate;  
    ...
    filterTableData(event){
		...
		if(event){
            ...
            this.highestPage = 1;
            if(!this.comunidadCashNexus) this.sendDataServiceRequestWithFilters(filters);
            else{
                var isEndOfDay = !this.lastUpdate;
