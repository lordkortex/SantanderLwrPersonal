...
export default class Lwc_cn_pagination extends LightningElement {
    ...
    get getPage(){
        if(this.pagelist){
            return ((this.pagelist.length > this.currentPage+this.pagesToShow-1 || this.currentPage==this.end) && this.currentPage != this.pagelist.length);
        }
    }

    get pageListLengthGreaterThan(){
        if(this.pagelist){
            return ((this.pagelist.length > this.pagesToShow) && ((this.currentPage > 1) && this.start != 0));
        }
    }

    get retrievedGreater100000(){
        return (this.namelisttodisplay == this.label.Account_Transactions || this.namelisttodisplay == this.label.Movements) && this.totalRetrieved > 10000;
    }
