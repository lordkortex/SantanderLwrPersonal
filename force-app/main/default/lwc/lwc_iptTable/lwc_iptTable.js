...
export default class lwc_IPTTable extends NavigationMixin(LightningElement){
    ...
    get isJsonArrayEmpty() {
        return this.jsonArray.length == 0 || this.jsonArray == null || this.jsonArray == undefined ;
    }
    @api
    getData() {
        ...
        try{
            ...
            try {
                ...
                .catch((error) => {
                    console.log('### lwc_iptTable ### getData() ::: Catch (1): ' + JSON.stringify(error));
                    this.jsonArray = [];
    ...
    sortBy(event) {
        try {
            ...
            var desc = this.template.querySelector("#"+event.target.id).getAttribute('data-value');
            if(desc != null && desc != "" && desc != undefined){
                var targetItem = (desc).includes('valueDate') ? 'valueDate': 'settledAmount';
                var sortItem = (desc).includes('valueDate') ? this.sortvalueDate : this.sortsettledAmount; // GAA Quiero que sortItem sea el valor del attributo llamado sort+targetItem, no el string en sí
                ...
                if (sorted != undefined && sorted !=null){
                    ...
                    if(targetItem.includes('valueDate')){
                        ...
                        this.sortsettledAmount = this.sortsettledAmount == 'desc' ? 'asc' : 'desc';
                    }
                    this.currentPage = 1;
    ...
    openPaymentDetails(event) {
        ...
        const item = this.jsonArray.find(function (item) { 
         ...
         +'&c__beneficiaryCountry='+item.paymentDetail.beneficiaryData.creditorAgent.agentCountry+'&c__filters='+this.filters+'&c__allAccounts='+JSON.stringify(this.fullaccountlist);
    ...
    setPaymentStatus(payments) {
        for(var key in payments){
            ...
            if(status=='ACSP'){
                if(reason=='G000' || reason =='G001' || reason==null || reason=='null'){
                    payments[key].paymentDetail.statusLabel = this.label.payment_statusThree;
                    payments[key].paymentDetail.statusClass = "circle icon-circle__blue";
                }
                if(reason=='G002' || reason =='G003' || reason =='G004'){
                    payments[key].paymentDetail.statusLabel = this.label.payment_statusFour;
                    payments[key].paymentDetail.statusClass = "circle icon-circle__orange";
                }
            }
        }
        return payments;
    ...
    getDateTime(){
        try{
            if(item.paymentDetail.statusDate != undefined){
                ...
                .then((result) => {
                    var results = res.substring(8,10) +"/"+result.substring(5,7)+"/"+result.substring(0,4) +" | "+result.substring(11);
