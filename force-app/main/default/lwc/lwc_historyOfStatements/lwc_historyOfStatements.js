...
export default class Lwc_historyOfStatements extends LightningElement {
        ...
        if(response != "") {
            ...
            for(var i = 0; i < response.length ; i++) {
                ...
                switch(sParameterName[0]) {
                    case("c__comesFrom"):
                        sParameterName[1] === undefined ? 'Not found' : this.comesFrom = sParameterName[1];
                        var listObject = [];
                        listObject.push({});
                        ...
                        break;  
                    case("c__accountName"):
                        sParameterName[1] === undefined ? 'Not found' :  this.accountName = sParameterName[1];
                        break;
                    case("c__currentCurrency"):
                        sParameterName[1] === undefined ? 'Not found' : this.accountCurrency = sParameterName[1];
                        break;
                    case("c__accountNumber"):
                        sParameterName[1] === undefined ? 'Not found' : this.accountNumber = sParameterName[1];
                        break;
                    case("c__bankName"):
                        sParameterName[1] === undefined ? this.bankName = "" : this.bankName = sParameterName[1];
                        break;
                    case("c__accountCode"):
                        sParameterName[1] === undefined ? 'Not found' : this.accountCode = sParameterName[1];
                        break;
                    case("c__subsidiaryName"):
                    sParameterName[1] === undefined ? this.subsidiaryName = "" : this.subsidiaryName = sParameterName[1];
                        break;
                }
            }  
        }
    ...
    getISOStringFromDateString(dateString, codCuenta){
        getCurrentUserTimezoneOffSetInMiliseconds({ dateInput : [dateString[0],dateString[1]] })
        .then(response =>{
            var res = response;
            if(res != null && res != undefined && res != ""){
                var from = this.formatDateGMT(dateString[0], res[dateString[0]], true);
    ...
    handleSearch(event) {
        ...
        this.loading = true;
        if(this.comesFrom != "Accounts"){
            var selectedAccount = (this.selectedAccount.includes('-') ? this.selectedAccount.split('- ')[1] : this.selectedAccount);
    
            var matchingAccount = this.accountsList.filter( data => data['displayNumber'] == selectedAccount);
            this.selectedAccountObject = matchingAccount;
            var dates = this.dates;
            
            this.accountCurrency = (this.selectedAccount.includes('-') ? this.selectedAccount.split('- ')[0] : "");
            ...
            if(datesOK){
                ...
                if(this.selectedAccountObject == null || this.selectedAccountObject == undefined){
