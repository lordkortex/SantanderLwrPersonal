    ...
    getStatus : function(component, event, helper) {
        ...
        action.setCallback(this, function(response) {
            ...
            if (state === "SUCCESS") {
                ...
                var arrayMapKeys = [];
                let statusListAux = [];
                let wholeStatusListAux = [];
                var wholeArrayMapKeys = [];
                if(!$A.util.isEmpty(result)){
                    for(let i=0; i< result.length; i++){   
                        if(!statusListAux.includes(result[i].parsedStatusName)){
                            statusListAux.push(result[i].parsedStatusName);
                            arrayMapKeys.push({
                                'label' : result[i].parsedStatusName,
                                'value' : result[i].parsedStatusName
                            }); 
                        }
                        
                        if(!wholeStatusListAux.includes(result[i].statusName)){
                            wholeStatusListAux.push(result[i].statusName);
                            wholeArrayMapKeys.push({
                                'label' : result[i].parsedStatusName,
                                'value' : result[i].parsedStatusName
                            }); 
                        }
                ...
                component.set('v.wholeStatusList', wholeArrayMapKeys);
                 resolve('La ejecucion ha sido correcta.'); 
            }
            else if (state === "INCOMPLETE") {
                reject($A.get('$Label.c.ERROR_NOT_RETRIEVED'));
                ...
                for (let i = 0; i < f_paymentStatusList.length; i++) {
                    ...
                    if(!$A.util.isEmpty(item)){
                        for(let i = 0; i < item.length; i++){
                            statusList.push({
                            "status": item[i].PAY_PCK_Status_code__c,
                            "reason" : item[i].PAY_PCK_Reason_code__c
                        	});  
                        }
                    }
    ...
    */
    findStatusReasonRecord : function (component, event, helper, value) {
        ...
        if(!$A.util.isEmpty(paymentStatusList)){
            record = paymentStatusList.filter(item => item.PAY_PCK_Status_Name__c === value);
        }else{
            record = {}
        }
        return record;
