({
            ...
            if (returnValue.success == true) {
              ...
              if (
                ...
                helper.showToast(
                  ...
                  true,
                  "error"
                ); // codigo previo a modificaciones
              ...
              helper.showToast(
                ...
                true,
                "error"
              ); // JHM Pendiente de revisar para el urlparam, posterior a modificaciones
              reject("ko");
            }
  ...
  connectCometd: function (component, event, helper) {
    ...
    cometd.handshake(function (handshakeReply) {
      if (handshakeReply.successful) {
        ...
        let newSubscription = cometd.subscribe(
          ...
          if (
            ...
            if (signature.lastSign == "true") {
              helper
                ...
                .then(
                  $A.getCallback(function (value) {
                    helper.sendToLanding(component, event, helper, true, false);
                  })
                )
                .catch(
                  $A.getCallback(function (error) {
      ...
      $A.getCallback(function (resolve, reject) {
        ...
        if (component.get("v.showOTP") == false) {
          if (signature.signatory == "true" && signature.signed == "false") {
            if (signature.lastSign == "true") {
              helper
                .reloadFXValue(component, event, helper, false)
                .then(
                  $A.getCallback(function (value) {
  ...
  reloadFXValue: function (component, event, helper, feesBoolean) {
    return new Promise(
      $A.getCallback(function (resolve, reject) {
        let paymentDraft = component.get("v.paymentDraft");
        let currencyCodeAvailableBalanceOrigin =
          paymentDraft.sourceAccount.currencyCodeAvailableBalance;
        let currencyCodeAvailableBalanceDestination =
          paymentDraft.destinationAccount.currencyCodeAvailableBalance;
        if (
          feesBoolean == true &&
          $A.util.isEmpty(paymentDraft.convertedTransactionFee)
        ) {
          resolve("ok");
        } else if (
          feesBoolean == false &&
          currencyCodeAvailableBalanceOrigin ==
            currencyCodeAvailableBalanceDestination
        ) {
          resolve("ok");
        } else {
              ...
              if (stateRV.success) {
                if (feesBoolean == true) {
                  if (!$A.util.isEmpty(stateRV.value.convertedAmount)) {
                    paymentDraft.convertedTransactionFee =
                      stateRV.value.convertedAmount;
                  }
                  if (!$A.util.isEmpty(stateRV.value.output)) {
                    paymentDraft.convertedTransactionFeeServiceResponse =
                      stateRV.value.output;
                  }
                } else {
                  component.set(
                    "v.exchangeRateLabel",
                    $A.get("$Label.c.exchangeRate")
                  );
  ...
  getTotalAmount: function (component, event, helper) {
    ...
    let currencyCodeAvailableBalance =
      paymentDraft.currencyCodeAvailableBalance;
    let fee = paymentDraft.transactionFee;
    let total = 0;
    if (
      !$A.util.isEmpty(convertedFee) &&
      !$A.util.isEmpty(convertedFeeCurrency)
    ) {
        ...
        action.setCallback(this, function (actionResult) {
          if (actionResult.getState() == "SUCCESS") {
            ...
            if (!returnValue.success) {
              ...
              if (
                $A.util.isEmpty(returnValue.value.initiateOTP.localSigningUrl)
              ) {
    ...
    return new Promise(
      $A.getCallback(function (resolve, reject) {
        ...
        if (
          ...
          $A.util.isEmpty(paymentDraft.reference)
        ) {
          reject("KO");
        } else {
