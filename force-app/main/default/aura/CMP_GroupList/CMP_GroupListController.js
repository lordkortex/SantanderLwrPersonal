    ...
    */
    ...
    navigateToProfile : function(component, event, helper) {
        ...
        for(var i = 0; i < groupList.length; i++) {
            ...
            if(groupList[i].datosGrupos.codgrupr == selectedGroup) {
                var nameGroup = groupList[i].datosGrupos.nomgruus;
                var codGroup = groupList[i].datosGrupos.codgrupr;
                var idAuth = groupList[i].datosGrupos.idauth;
            }
        }

        var url = "c__groupName="+nameGroup+
        "&c__comesFrom="+"Groups"+
        "&c__groupId="+codGroup+
        "&c__groupAuth="+idAuth+
    ...
    navigateToView : function(component, event, helper) {
        ...
        var url = "c__userGroup="+nameGroup+
        "&c__comesFrom="+"Groups"+
        "&c__groupId="+codGroup+
        "&c__groupAuth="+idAuth;
        helper.encrypt(component, url).then(function(results)
    ...
    deleteGroup : function(component, event, helper) {
        ...
        eventDelete.setParams({
                titleDelete : title,
                secondDescriptionDelete : secondDescription.replace('{NOMBRE DEL GRUPO}',nameGroup),
                firstDescriptionDelete : nameGroup,
