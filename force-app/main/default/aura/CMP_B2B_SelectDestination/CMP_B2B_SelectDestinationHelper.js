({
    ...
    validateAccount: function (component, event, helper) {
        return new Promise($A.getCallback(function (resolve, reject) {
            let paymentDraftAux = component.get('v.paymentDraft');
            let userDataAux = component.get('v.userData');
                ...
                if (actionResult.getState() == 'SUCCESS') {
                    ...
                    if (!returnValue.success) {
                        ...
                        if (returnValue.value.statusResult != 'OK') {
                            component.set('v.errorMSG_2', $A.get('$Label.c.blockedAccount'));
                            //helper.showToast(component, notificationTitle, bodyText, true);
                            reject('ko');
                        } else {
                            resolve('ok');
    ...
    */
    ...
    validateNewBeneficiary: function (component, event, helper) {
        return new Promise($A.getCallback(function (resolve, reject) {
            let paymentDraft = component.get('v.paymentDraft');
            let newBeneficiary = component.get('v.draftData');
            let action = component.get('c.validateBeneficiaryData');
            action.setParams({
                'newAccount': newBeneficiary
            });
            action.setCallback(this, function (response) {
                ...
                if (state === 'SUCCESS') {
                    ...
                    if (result.success == true) {
                        let paymentDraft = component.get('v.paymentDraft');
                        paymentDraft.destinationAccount = result.value.accountData;
                        paymentDraft.registerNewBeneficiary = true;
                        component.set('v.paymentDraft', paymentDraft);
                        
                        newBeneficiary.branch = paymentDraft.destinationAccount.branch;
                        newBeneficiary.displayNumber = paymentDraft.destinationAccount.displayNumber;
                        component.set('v.draftData', newBeneficiary);
                        resolve('OK');
                    } else {
                        component.set('v.errorMSG_1', result.msg);
                        reject('Cannot create new beneficiary');
                    }
                } else {
                    let notificationTitle = $A.get('$Label.c.B2B_Error_Problem_Loading');
    ...
    */
    setCountryList: function (component, event, helper, countryList) {
        return new Promise($A.getCallback(function (resolve, reject) {
            ...
            var sortCountryList = helper.sortList(component, helper, outputCountryList);
    ...
    */
    handleCurrencies: function (component, event, helper, country) {
        return new Promise($A.getCallback(function (resolve, reject) {
            ...
            if (canNewBen == false) {
                ...
                helper.setCurrencyList(component, event, helper, currencyList)
                .then($A.getCallback(function (value) {
                    return helper.setSelectedCurrency(component, event, helper, baseCurrency)
            ...
            action.setCallback(this, function (response) {
                ...
                if (response.getState() == 'SUCCESS') {
                    ...
                    if (returnValue.success) {
                        ...
                        console.log('fx');
                        console.log(value);
                        if (!$A.util.isEmpty(value.sameCurrencies) && value.sameCurrencies == true) {
    ...
    setCurrencyList: function (component, event, helper, inputCurrencyList) {
        return new Promise($A.getCallback(function (resolve, reject) {
            ...
            let sortedCurrencyList = helper.sortList(component, helper, currencyList);
    ...
    getDominantCurrency: function (component, helper, currencyOrigin, currencyDestination) {
        return new Promise($A.getCallback(function (resolve, reject) {
            let paymentDraft = component.get('v.paymentDraft');
            let userData = component.get('v.userData');
            if (!$A.util.isEmpty(currencyOrigin) && !$A.util.isEmpty(currencyDestination)) {
    ...
    */
    checkExchangeRate: function (component, helper) {
        return new Promise($A.getCallback(function (resolve, reject) {
            ...
            if ($A.util.isEmpty(paymentDraft.sourceAccount)) {
            ...
            } else if ($A.util.isEmpty(paymentDraft.sourceAccount.currencyCodeAvailableBalance) || $A.util.isEmpty(paymentDraft.paymentCurrency)) {
                ...
                if ($A.util.isEmpty(paymentDraft.exchangeRate) || $A.util.isEmpty(paymentDraft.sourceCurrencyDominant)) {
                    return helper.getExchangeRate(component, helper, paymentDraft.sourceAccount.currencyCodeAvailableBalance, paymentDraft.paymentCurrency)
                        .then($A.getCallback(function (value) {
                            return helper.getDominantCurrency(component, helper, paymentDraft.sourceAccount.currencyCodeAvailableBalance, paymentDraft.paymentCurrency)
                        })).then($A.getCallback(function (value) {
                            resolve('OK');
                        }));
                } else {
    ...
    */
        ...
        return new Promise($A.getCallback(function (resolve, reject) {
            ...
            if(!$A.util.isEmpty(paymentCurrency)) {
                ...
                if (!$A.util.isEmpty(selectedCurrency)) {
                    ...
                    }else if ($A.util.isEmpty(paymentDraft.destinationAccount.currencyCodeAvailableBalance)) {
        ...
        if (canNewBen == true) {
            ...
            .then($A.getCallback(function(value) {
                ...
                if (value != selectedCurrency) {
                    return helper.handleChangeCurrency(component, event, helper, value);
                }else {
                    return ('ok');
                }
            })).then($A.getCallback(function(value) {
                ...
                let paymentDraft = component.get('v.paymentDraft');
                if (!$A.util.isEmpty(paymentDraft.destinationAccount) && !$A.util.isEmpty(paymentDraft.destinationAccount.country)) {
    ...
    handleChangeCurrency: function(component, event, helper, currency) {
        let notificationTitle =  $A.get('$Label.c.SorryProblem');
        let bodyText =  $A.get('$Label.c.PAY_TryAgain');
        component.set('v.spinner', true);
        let paymentDraft = component.get('v.paymentDraft');
        let sourceAccount = paymentDraft.sourceAccount;
