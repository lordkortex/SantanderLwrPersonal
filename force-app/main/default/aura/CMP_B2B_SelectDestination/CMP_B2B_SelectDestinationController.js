({
  initComponent: function (component, event, helper) {
    ...
    if (transferType == $A.get("$Label.c.PTT_international_transfer_single")) {
      ...
      helper
        .getCountryList(component, event, helper)
        .then(
        ...
        .then(
          $A.getCallback(function (value) {
            return helper.getNewBeneficiarySettings(
              component,
              event,
              helper,
              value
  ...
  handleCheckContinue: function (component, event, helper) {
    ...
    if ($A.util.isEmpty(paymentDraft.destinationAccount)) {
      if (!$A.util.isEmpty(draftData)) {
        helper
          .recordNewBeneficiary(component, event, helper)
          .then(
            $A.getCallback(function (value) {
        ...
        component.set(
          "v.errorMSG_1",
          $A.get("$Label.c.B2B_Error_Enter_Input").replace("{0}.", "")
        );
        component.set(
          "v.errorMSG_2",
          $A.get("$Label.c.B2B_Error_Invalid_Input").replace("{0} format.", "")
        );
      ...
      helper
        .validateAccount(component, event, helper)
        .then(
          $A.getCallback(function (value) {
            return helper.setPaymentCurrency(component, helper);
          })
