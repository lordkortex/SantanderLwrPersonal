({
        ...
        try {
            ...
            action.setCallback(this, function(response) {
                ...
                if (state == "ERROR") {
                    ...
                    helper.initTable(component, event, response.getReturnValue(), false, helper);
                }    
    ...
    initTable : function(component, event, res, hideSpinner, helper){
        if(res != null && res != undefined && Object.keys(res).length>0){
            ...
            parseJSON = JSON.parse(JSON.stringify(helper.setPaymentStatus(parseJSON)));
    ...
    setPaymentStatus : function(payments){
        for(var key in payments){
            ...
            if(status=='RJCT'){
                payments[key].paymentDetail.statusLabel = $A.get("$Label.c.payment_statusOne");
                payments[key].paymentDetail.statusClass = "icon-circle__red";
            }
            if(status=='ACSC' || status=='ACCC'){
                payments[key].paymentDetail.statusLabel = $A.get("$Label.c.payment_statusTwo");
                payments[key].paymentDetail.statusClass = "icon-circle__green";
            }
            if(status=='ACSP'){
                if(reason=='G000' || reason =='G001' || reason==null || reason=='null'){
                    payments[key].paymentDetail.statusLabel = $A.get("$Label.c.payment_statusThree");
                    payments[key].paymentDetail.statusClass = "icon-circle__blue";
                }
                if(reason=='G002' || reason =='G003' || reason =='G004'){
                    payments[key].paymentDetail.statusLabel = $A.get("$Label.c.payment_statusFour");
