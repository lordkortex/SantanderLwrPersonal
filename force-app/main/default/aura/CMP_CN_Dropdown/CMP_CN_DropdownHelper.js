({
            ...
            for(var item in item_aux){
                if(item_aux.includes(component.get("v.selectAllValues"))){
                    ...
                    if(document.getElementById(item_aux[item]).classList.contains("slds-is-selected")){
                        ...
                        if(selectedValuesList.indexOf(component.get("v.selectAllValues")) != -1){
                            document.getElementById(component.get("v.selectAllValues")).classList.remove("slds-is-selected");
                            selectedValuesList.splice(selectedValuesList.indexOf(component.get("v.selectAllValues")),1);
                        }
                    }else{
    ...
    keepSelection : function(component, event, items){

        // for(var i = 0; i < items.length; i++){
            console.log(">>>>>>>" + document.getElementById(items).classList + ">>>>>>>");
            //if (!document.getElementById(items).classList.contains("slds-is-selected")) {
                document.getElementById(items).classList.add("slds-is-selected");
                console.log(">>>>>>>" + document.getElementById(items).classList + ">>>>>>>")
            //}
        // }
    }
