({
    ...
    */
   showInformation: function (component, event, helper, amountEntered, amountEnteredFrom) {
        return new Promise($A.getCallback(function (resolve, reject) {
            ...
            if (CMP_amountSend != null && CMP_amountSend != undefined && amountEnteredFrom == 'source') {
                CMP_amountSend.setInputAmount('sourceAmountInput', amountEntered);
            }
            if (!$A.util.isEmpty(sourceAccount) && !$A.util.isEmpty(destinationAccount) && amountEnteredFrom == 'recipient') {
                if (!$A.util.isEmpty(sourceAccount.currencyCodeAvailableBalance) && !$A.util.isEmpty(destinationAccount.currencyCodeAvailableBalance)) {
                    if (sourceAccount.currencyCodeAvailableBalance != destinationAccount.currencyCodeAvailableBalance) {
                        ...
                        if (CMP_amountReceive != null && CMP_amountReceive != undefined) {
                            CMP_amountReceive.setInputAmount('recipientAmountInput', amountEntered);
                        }
                    }
                }
            }
    ...
    29/03/2021		Antonio Duarte				PaymentDraft version
    */
    formatUpdatedDate: function (component, event, helper) {
        let paymentDraft = component.get('v.paymentDraft');
        let sourceAccountData = paymentDraft.sourceAccount;
        let recipientAccountData = paymentDraft.destinationAccount;
    ...
    */
    ...
    paymentDetailsContinue : function (component, event, helper) {
        return new Promise(function (resolve, reject) {
            ...
            let reference =  $A.util.isEmpty(paymentDraft.reference) ? '' : paymentDraft.reference;
            let purpose =  $A.util.isEmpty(paymentDraft.purpose)  ? '' : paymentDraft.purpose;
            let charge =  $A.util.isEmpty(paymentDraft.chargeBearer)  ? '' : paymentDraft.chargeBearer;
            let paymentReason = $A.util.isEmpty(paymentDraft.reason)  ? '' : paymentDraft.reason;
            let clientReference = '' ;
            ...
            let description =  $A.util.isEmpty(paymentDraft.description)  ? '' : paymentDraft.description;
    ...
    updatePaymentDetails: function (component, helper) {
        return new Promise($A.getCallback(function (resolve, reject) {
            ...
            /*if (!$A.util.isEmpty(paymentDraft)) {
                ...
                if (!$A.util.isEmpty(paymentDraft.serviceId)) {
                    serviceId = paymentDraft.serviceId;
                }
            }*/
            paymentDraft.chargeBearer = 'OUR'; // Siempre 'OUR' para Book To Book. Posibles valores 'OUR'-nuestro, 'SHA'-compartido, 'BEN'-recipiente
            component.set('v.paymentDraft', paymentDraft);
    ...
    */
    ...
    initComponent: function (component,event,helper) {
        return new Promise($A.getCallback(function (resolve, reject) {
            ...
            let paymentDraft = component.get('v.paymentDraft');
            if(!$A.util.isEmpty(paymentDraft) && paymentDraft.sourceAccount != null && paymentDraft.destinationAccount != null && !$A.util.isEmpty(paymentDraft.sourceCurrencyDominant) && !$A.util.isEmpty(paymentDraft.exchangeRate)){
                if(!$A.util.isEmpty(paymentDraft.sourceAccount.currencyCodeAvailableBalance) && !$A.util.isEmpty(paymentDraft.paymentCurrency)){
 ...
 /*             var action = component.get('c.getPaymentDetail');
                    ...
                    if (actionResult.getState() == 'ERROR') {
                        ...
                        if (stateRV.success) {                        
                            ...
                            if(stateRV.value.paymentPurposeValues != null){
                                let purposeList = stateRV.value.paymentPurposeValues;
                                /*
    ...
    setShowBalanceInfo: function (component, event, helper) {
        return new Promise($A.getCallback(function (resolve, reject) {
            let transferType = component.get('v.transferType');
            let instantTransferValue = $A.get('$Label.c.PTT_instant_transfer');
            if(transferType == instantTransferValue){
                component.set('v.showBeneficiaryBalanceInfo', true);
            }
            resolve('ok');
        }), this);
    }
