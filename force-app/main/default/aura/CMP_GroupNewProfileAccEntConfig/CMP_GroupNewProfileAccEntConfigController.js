({
	...
	handleMultipicklistValues : function(component, event) {
		...
		if(event.getSource().getLocalId() == "accountsCombo"){
			if(params){
				...
				if(!params.selected) {
					accountData.unfiltered.available.push(params.valueObj);
					accountData.unfiltered.selected = accountData.unfiltered.selected.filter(account => account.value != params.value);
					component.set("v.accountData.unfiltered", accountData.unfiltered);

					// Display all the entitlements values
					if(params.valueObj.value == $A.get("$Label.c.ServiceProfiling_EntitltementNoAccount")){
				...
				// The unfiltered available value must be moved to selected
					...
					accountData.unfiltered.selected.push(params.valueObj);
					accountData.unfiltered.available = accountData.unfiltered.available.filter(account => account.value != params.value);
					component.set("v.accountData.unfiltered", accountData.unfiltered);

					// Filter the entitlements when "Entitlement with no account" is selected
					if(params.valueObj.value == $A.get("$Label.c.ServiceProfiling_EntitltementNoAccount")){
						entitlementData.filtered.available = entitlementData.unfiltered.available.filter(ent => ent.value == $A.get("$Label.c.ServiceProfiling_DowRepSicContingency"));
						entitlementData.filtered.selected = entitlementData.unfiltered.selected.filter(ent => ent.value == $A.get("$Label.c.ServiceProfiling_DowRepSicContingency"));
			...
			if(params){
				if(!params.selected){
					entitlementData.unfiltered.available.push(params.valueObj);
					entitlementData.unfiltered.selected = entitlementData.unfiltered.selected.filter(ent => ent.value != params.value);		
				// Value moved from left to right
				} else {
					entitlementData.unfiltered.selected.push(params.valueObj);
					entitlementData.unfiltered.available = entitlementData.unfiltered.available.filter(ent => ent.value != params.value);
				}
		...
		if(accountList.length > 0 && entitlementList.length > 0){
			for(var acc in accountList){
				...
				for(var ent in entitlementList){
					entitlements.push(entitlementList[ent].displayValue);
					data.entitlement = entitlements;
				}
				data.account = accountList[acc].displayValue;
			...
			if(params.filterDataTable == false && params.filterByCountry != undefined){
				if(params.filterByCountry == $A.get("$Label.c.ServiceProfiling_AllCountries")) {
					...
					component.set("v.accountData.filtered.available" , 
									component.get("v.accountData.unfiltered.available").filter(account => account.countryCode == params.filterByCountry));
					component.set("v.accountData.filtered.selected" , 
									component.get("v.accountData.unfiltered.selected").filter(account => account.countryCode == params.filterByCountry));
				}
			}
		}
	},
	...
	*/
	select : function(component, event, helper){
		...
		if(params){
			if(params.type == "account" && params.values.length > 0){
				component.set("v.accountData.filtered.available" , 
									component.get("v.accountData.filtered.available").filter(account => !params.values.includes(account)));
				component.set("v.accountData.unfiltered.available" , 
									component.get("v.accountData.unfiltered.available").filter(account => !params.values.includes(account)));
				let selectedUnfilteredAccounts = component.get("v.accountData.unfiltered.selected");
				let selectedFilteredAccounts = component.get("v.accountData.filtered.selected");
				selectedUnfilteredAccounts.push(params.values);
				selectedFilteredAccounts.push(params.values);
				component.set("v.accountData.unfiltered.selected" , selectedUnfilteredAccounts);
				component.set("v.accountData.filtered.selected" , selectedFilteredAccounts);
			} else if(params.type == "entitlement" && params.values.length > 0){
				component.set("v.entitlementData.filtered.available" , 
									component.get("v.entitlementData.filtered.available").filter(ent => !params.values.includes(ent)));
				component.set("v.entitlementData.unfiltered.available" , 
									component.get("v.entitlementData.unfiltered.available").filter(ent => !params.values.includes(ent)));
				let selectedUnfilteredEntitlements = component.get("v.entitlementData.unfiltered.selected");
				let selectedFilteredEntitlements = component.get("v.entitlementData.filtered.selected");
				selectedUnfilteredEntitlements.push(params.values);
				selectedFilteredEntitlements.push(params.values);
				component.set("v.entitlementData.unfiltered.selected" , selectedUnfilteredEntitlements);
				component.set("v.entitlementData.filtered.selected" , selectedFilteredEntitlements);
			}
		}
	}
})