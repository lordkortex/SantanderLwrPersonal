({
                        ...
                        .then($A.getCallback(function (results) {
                            ...
                            for (var i = 0; i < sURLVariables.length; i++) {
                                ...
                                if (sParameterName[0] === 'c__paymentId') { 
                                    sParameterName[1] === undefined ? 'Not found' : paymentId = component.set('v.paymentId',sParameterName[1]);
                                }
                                if (sParameterName[0] === 'c__source') {
                                    sParameterName[1] === undefined ? 'Not found' : source = component.set('v.source',sParameterName[1]);
                                }
                                ...
                                if (sParameterName[0] === 'c__signatoryDetails') {
                                    sParameterName[1] === undefined ? 'Not found' : component.set('v.signLevel',JSON.parse(sParameterName[1]));
                                }
                                if (sParameterName[0] === 'c__signId') {
                                    sParameterName[1] === undefined ? 'Not found' : component.set('v.scaUid',sParameterName[1]);
                                }
                            }
                            if ($A.util.isEmpty(paymentDetails) && component.get("v.paymentDraft") == null) {
                                helper.getPaymentData(component, event, helper)
                                .then($A.getCallback(function (value) {
                                    resolve('ok'); 
                                }));
                            } else {
                                if($A.util.isEmpty( component.get("v.paymentDraft"))){
    ...
    */
    ...
    handleExecutePayment: function (component, event, helper) {
        return new Promise($A.getCallback(function (resolve, reject) {
            ...
            let FXTimer = ($A.util.isEmpty(paymentData.FXDateTime) ? null : paymentData.FXDateTime);
            let FXData = ($A.util.isEmpty(paymentData.FXoutput) ? null : paymentData.FXoutput);
            let feesData = ($A.util.isEmpty(paymentData.FXFeesOutput) ? null : paymentData.FXFeesOutput);
    ...
    */
    ...
    sendOTP_Strategic: function (component, event, helper) {
                ...
                if (actionResult.getState() == 'SUCCESS') {
                    ...
                    if (!returnValue.success) {
                        ...
                        if(returnValue.value.initiateOTP.localSigningUrl == null ||returnValue.value.initiateOTP.localSigningUrl == undefined ){
                            ...
                            let signatureMode = component.get("v.signatureMode");
                            let sameWindow = System.Label.CNF_SignatureMode_003;
                            if(signatureMode == sameWindow){
                                 var win = window.open(returnValue.value.initiateOTP.localSigningUrl, '_self');
                            }else{
     ...
     postFraudAuthorization: function (component, event, helper, callTypeParam) {
        return new Promise(function (resolve, reject) {
            try {
                ...
                var paymentDraft = component.get('v.paymentData');
                action.setParams({
                    userData : userData,
    ...
    */
    ...
    reloadFXValue: function (component, event,  helper, feesBoolean) {
        return new Promise($A.getCallback(function (resolve, reject) {
            ...
            if (feesBoolean == true && (($A.util.isEmpty(paymentFees) || $A.util.isEmpty(feesCurrency)) || (!$A.util.isEmpty(paymentFees) && (payment.sourceCurrency == feesCurrency)))) {
                resolve('ok');
   ...
   encrypt: function (component, data) {
    ...
    Company:        Deloitte
    Description:    Set the values of the paymentData
    History:
    <Date>          <Author>            <Description>
    04/03/2021      Andrea Martin          Initial version 
    */
    getPaymentDataFrom : function (component, event, helper){
        return new Promise($A.getCallback(function (resolve, reject) {
            let paymentDraft = component.get("v.paymentDraft");
            if(!$A.util.isEmpty(paymentDraft)){
                //from the process 
                var action = component.get('c.getPaymentFromPD');
                action.setParams({
                    'paymentDraft': paymentDraft
                });
                action.setCallback(this, function (actionResult) {
                    if(actionResult.getState() == 'SUCCESS'){
                        ...
                        console.log(returnValue.value.paymentDetail);
                        component.set('v.paymentData', returnValue.value.paymentDetail);
                        component.set('v.paymentId',returnValue.value.paymentDetail.paymentId);
                        resolve('ok');
                    }else{
    ...
    Author:         Andrea Martin
    Company:        Deloitte
    Description:    Obtains the type of signature according to the type of user and its Country Origin
    History:
    <Date>          <Author>            <Description>
    17/03/2021      Andrea Martin          Initial version 
    */
    getSignatureMode : function (component, event, helper){
        return new Promise($A.getCallback(function (resolve, reject) {
            let paymentData = component.get("v.paymentData");
            let country = paymentData.sourceData.country;
            var action = component.get('c.getSignatureMode');
            action.setParams({
                'country': country
            });
            action.setCallback(this, function (actionResult) {
                if(actionResult.getState() == 'SUCCESS'){
                    var returnValue = actionResult.getReturnValue();
                    component.set('v.signatureMode', returnValue.value.signatureMode);
                    resolve('ok');
                }else{
                    console.error(actionResult);
                    reject('ko');
