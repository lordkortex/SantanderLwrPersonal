...
export default class Lwc_historyOfStatementsTable extends LightningElement {
  ...
  get statementsresultsNotNull() {
    return (
      this._statementslist == null ||
      this._statementslist === "" ||
      !(this._statementslist.length > 0)
    );
  }

  get statementsListEmpty() {
    return (
      JSON.stringify(this._statementslist) == JSON.stringify({}) ||
      this.statementslist == undefined ||
      !(this._statementslist.length > 0)
    );
  }
  ...
  get statementsListFromStartToEnd() {
    var listaAux = JSON.parse(JSON.stringify(this._statementslist)).slice(
      this.start,
      this.end
    );
    Object.keys(listaAux).forEach((key) => {
      listaAux[key].index = key;
      listaAux[key].valueBalance_Formatted =
        listaAux[key].saldo.valueBalance_Formatted + " " + this.accountcurrency;
      listaAux[key].bookBalance_Formatted =
        listaAux[key].saldo.bookBalance_Formatted + " " + this.accountcurrency;
    });
    return listaAux;
  }
  ...
  navigateToMovements(event) {
    var selectedExtract = this._statementslist[event.currentTarget.dataset.id]
      .saldo;
    ...
    var url =
      ...
      "&c__accountCurrency=" +
      account.currencyCodeAvailableBalance +
      "&c__bookBalance=" +
      selectedExtract.bookBalance_Formatted +
      "&c__valueBalance=" +
      selectedExtract.valueBalance_Formatted;
    ...
    var data = { page: "statement-movement", urlParams: url };
