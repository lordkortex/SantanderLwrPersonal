...
export default class Lwc_cn_filters extends LightningElement {
  ...
  setFilterAux(filters) {
    if (filters) {
      ...
      Object.keys(filters).forEach((key) => {
        filtersAux[key].label1 =
          filtersAux[key].name + " (" + filtersAux[key].numberChecked + ")";
        ...
        filtersAux[key].filterNumberCheckedZero =
          filtersAux[key].numberChecked == 0;
        filtersAux[key].filterNumberCheckedGt =
          filtersAux[key].numberChecked > 0;
      ...
      for (var key in filters) {
        if (filters[key].name == filterName.split("-")[0]) {
          if (filters[key].type == "checkbox") {
            ...
            //Remove error
            if (
              this.template.querySelector('[data-id="dateFromInput"]') !=
                null &&
              this.template.querySelector('[data-id="dateToInput"]') != null
            ) {
              this.template
    ...
    if (filterName) {
      ...
      for (var key in filters) {
        if (filters[key].name == filterName) {
          if (
            filters[key].displayOptions == undefined ||
      ...
      if (this.template.querySelector('[data-id="' + buttonId + '"]') != null) {
        this.template
          .querySelector('[data-id="' + buttonId + '"]')
          .classList.toggle("buttonSelected");
      }
    }
  }
  ...
  openModal() {
    const compEvent = new CustomEvent("openmodal", {
    ...
    if (filterClicked == false) {
      ...
      for (var key in filters) {
        if (
          ...
          filters[key].name != clickedElement.id
        ) {
          filters[key].displayOptions = false;
        }
      }
  ...
  applyFilters(event) {
    ...
    if (
      event &&
      event.currentTarget &&
      event.currentTarget.name == "clearBtn"
    ) {
    ...
    if (isValidDate && !this.showAmountError && !this.showFormatError) {
      ...
      if (isValidAmount) {
        ...
        if (flagEvent == false) {
          ...
          const compEvent = new CustomEvent("firefilter", {
            detail: { cmpParams }
          });
          this.dispatchEvent(compEvent);
          if (event) {
  ...
  getISOStringFromDateString(dateString, selectedFilters) {
    try {
      ...
      console.error(
        "### lwc_cn_filters ### getISOStringFromDateString() ::: Catch Error: " +
          e
      );
    }
  }
  ...
  handledropdownvalueselected(evt) {
    ...
    const selectedEvent = new CustomEvent("dropdownvalueselected", {
      detail: this.dropdownselectedvalue
    });
