...
// Import apex methods
import downloadAccountsController from "@salesforce/apex/CNT_DownloadAccountsLUController.downloadAccountsController";
...
export default class Lwc_serviceComponent extends NavigationMixin(
  LightningElement
) {
  @api onCallApexD(data) {
    ...
    this.callApexD(callercomponent, controllermethod, actionparameters);
  }
  @api onCallApex(data) {
    console.log("onCallApex");
    var callercomponent = data.callercomponent;
  ...
  callApexD(callercomponent, controllerMethod, actionParameters) {
    downloadAccountsController({
      actualData: actionParameters.actualData,
      actualGrouping: actionParameters.actualGrouping,
      consolidationCurrency: actionParameters.consolidationcurrency,
      tiposDeCambio: actionParameters.tiposDeCambio
    })
      .then((result) => {
        console.log("OK");
  ...
  callApex(callercomponent, controllerMethod, actionParameters) {
    ...
    if (controllerMethod === "encryptData") {
    ...
    } else if (controllerMethod === "downloadAccountsController") {
      downloadAccountsController(actionParameters)
        .then((result) => {
          console.log("OK");
          const successcallback = new CustomEvent("successcallback", {
            detail: { callercomponent: callercomponent, value: result }
      ...
      controllerMethod === "getCurrentUserTimezoneOffSetInMiliseconds"
    ) {
    ...
    } else if (controllerMethod === "setUserCurrencyString") {
      setUserCurrencyString(actionParameters)
        .then((result) => {
          console.log("OK");
          const successcallback = new CustomEvent("successcallback", {
            detail: { callercomponent: callercomponent, value: result }
    ...
    } else if (controllerMethod === "updateOneTradeAlias") {
      updateOneTradeAlias(actionParameters)
        .then((result) => {
          console.log("OK");
          const successcallback = new CustomEvent("successcallback", {
            detail: { callercomponent: callercomponent, value: result }
  ...
  handleDecrypt(callercomponent, datauri) {
    ...
    if (sURLVariablesMain[0] == "params") {
      var result = this.decrypt(sURLVariablesMain[1]);
      const successcallback = new CustomEvent("successcallback", {
        detail: { callercomponent: callercomponent, value: result }
      });
      // Fire the custom event
  ...
  handleRetrieveFromCache(callercomponent, key) {
    ...
    let isFreshData =
      ...
      new Date() - new Date(Date.parse(timestamp)) <
        parseInt(refreshBalanceCollout) * 60000;
