...
export default class lwc_account_transactionsModalFormFilter extends LightningElement {
  ...
  get filtersIteration() {
    ...
    Object.keys(listAux).forEach((key) => {
      listAux[key].isCountryWithNooneChecked =
        listAux[key].name == this.label.Country &&
        listAux[key].numberChecked == 0;
      listAux[key].isCountryWithOneChecked =
        listAux[key].name == this.label.Country &&
        listAux[key].numberChecked == 1;
      listAux[key].isCountryWithGTOneChecked =
        listAux[key].name == this.label.Country &&
        listAux[key].numberChecked > 1;
      listAux[key].isBankWithNooneChecked =
        listAux[key].name == this.label.Bank && listAux[key].numberChecked == 0;
      listAux[key].isBankWithOneChecked =
        listAux[key].name == this.label.Bank && listAux[key].numberChecked == 1;
      listAux[key].isBankWithGTOneChecked =
        listAux[key].name == this.label.Bank && listAux[key].numberChecked > 1;
      listAux[key].isAccountWithNooneChecked =
        listAux[key].name == this.label.Account &&
        listAux[key].numberChecked == 0;
      listAux[key].isAccountWithOneChecked =
        listAux[key].name == this.label.Account &&
        listAux[key].numberChecked == 1;
      listAux[key].isAccountWithGTOneChecked =
        listAux[key].name == this.label.Account &&
        listAux[key].numberChecked > 1;
      listAux[key].isCurrencyWithNooneChecked =
        listAux[key].name == this.label.Currency &&
        listAux[key].numberChecked == 0;
      listAux[key].isCurrencyWithOneChecked =
        listAux[key].name == this.label.Currency &&
        listAux[key].numberChecked == 1;
      listAux[key].isCurrencyWithGTOneChecked =
        listAux[key].name == this.label.Currency &&
        listAux[key].numberChecked > 1;
      listAux[key].isCategoryWithNooneChecked =
        listAux[key].name == this.label.Category &&
        listAux[key].numberChecked == 0;
      listAux[key].isCategoryWithOneChecked =
        listAux[key].name == this.label.Category &&
        listAux[key].numberChecked == 1;
      listAux[key].isCategoryWithGTOneChecked =
        listAux[key].name == this.label.Category &&
        listAux[key].numberChecked > 1;
      ...
      listAux[key].itemNameIsCurrency =
        listAux[key].name == this.label.Currency;
      listAux[key].itemNameIsCategory =
        listAux[key].name == this.label.Category;
      listAux[key].itemNameIsAmount = listAux[key].name == this.label.amount;
      listAux[key].itemCheckedOptions =
        listAux[key].numberChecked + " " + this.label.options;
  ...
  showDropdown(event) {
    ...
    console.log(
      "###lwc_account_transactionsModalFormFilter ### showDropdown(event) ::: Id del Evento: " +
  ...
  chekOptions(event) {
    ...
    for (var key in filters) {
      if (filters[key].name == optionName && filters[key].type == "checkbox") {
        ...
        filters[key].numberChecked = filters[key].data.filter(
  ...
  saveData2(event) {
    var amountFrom = this.template.querySelector('[data-id="AmountFrom2"]');
    var amountTo = this.template.querySelector('[data-id="AmountTo2"]');
    ...
    if (amountFrom != null || amountTo != null) {
      ...
      for (var key in filters) {
        if (
          filters[key].name == this.label.amount &&
          ...
          if (
            this.amountBis[1] != undefined &&
            parseInt(this.amountBis[0]) > parseInt(this.amountBis[1])
          ) {
  ...
  getFormFilters(event) {
    ...
    console.log(
      "### lwc_account_transactionsModalFormFilter ### getFormFilters ::: Event Id: " +
        event.target.id +
        ", value: " +
        event.target.value +
        ", checked: " +
    ...
    if (inputId) {
      if (inputId.includes("debitInput")) {
        if (
          this.formFilters.debit == undefined ||
      ...
      if (inputId.includes("creditInput")) {
        if (
          this.formFilters.credit == undefined ||
  ...
  clearAll(event) {
    ...
    for (var key in filters) {
      if (filters[key].type == "checkbox") {
        ...
        //Remove error
        var dateFromIn = this.template.querySelector(
          '[data-id="dateFromInput"]'
        );
    ...
    try {
      ...
      console.log(
        "### lwc_account_transactionsModalFormFilter ### clearAll() ::: Error: " +
          e
      );
    }
  ...
  validateDate(event) {
    ...
    if (to != "") {
      if (to < from) {
        ...
        this.template
          .querySelector('[data-id="dateFromInput"]')
          .setCustomValidity(this.label.validationDate);
        this.template
          .querySelector('[data-id="dateFromInput"]')
          .reportValidity();
  ...
  calculateNumberActiveFilters() {
    ...
    for (var key in filters) {
      if (filters[key].type == "checkbox") {
        filterCount =
          filters[key].data.filter((option) => option.checked).length > 0
            ? filterCount + 1
            : filterCount;
      } else if (filters[key].type == "text") {
        filterCount =
          filters[key].selectedFilters != undefined &&
          ((filters[key].selectedFilters.from != undefined &&
            filters[key].selectedFilters.from != "") ||
            (filters[key].selectedFilters.to != undefined &&
              filters[key].selectedFilters.to != ""))
            ? filterCount + 1
            : filterCount;
      } else if (filters[key].type == "dates") {
        if (
          filters[key].data.length > 0 &&
          ((filters[key].data[0] != null &&
            filters[key].data[0] != undefined) ||
            (filters[key].data[1] != null && filters[key].data[1] != undefined))
        ) {
          filterCount++;
        } else {
          ...
          filterCount =
            datesBis.length > 0 &&
            ((datesBis[0] != null && datesBis[0] != undefined) ||
              (datesBis[1] != null && datesBis[1] != undefined))
              ? filterCount + 1
              : filterCount;
        }
      }
  ...
  checkDates(selectedFilters) {
    ...
    // Only To date is filled, then fill From with -25 months
    if (
      (dates[0] == undefined || dates[0] == "") &&
      dates[1] != undefined &&
      dates[1] != this.label.to
    ) {
      ...
      ((dates[1] == undefined || dates[1] == "") &&
        dates[0] != undefined &&
        dates[0] != this.label.from) ||
      new Date(Date.parse(dates[0])) > new Date(Date.parse(dates[1]))
    ) {
  ...
  validateAmount(event, from, to) {
    if (from != "" || to != "") {
      ...
      if (x > y) {
        ...
        (isNaN(from) && from != undefined) ||
        (isNaN(to) && to != undefined) ||
        x < 0 ||
        y < 0
      ) {
        this.showFormatError = true;
  ...
  applySearch(event) {
    ...
    if (
      event &&
      event.currentTarget &&
      event.currentTarget.id.includes("clearBtn")
    ) {
    ...
    if (
      this.template.querySelector('[data-id="dateFromInput"]') != undefined &&
      this.template.querySelector('[data-id="dateToInput"]') != undefined
    ) {
    ...
    if (isValidDate && !this.showAmountError && !this.showFormatError) {
      ...
      for (var key in filters) {
        if (filters[key].type == "checkbox") {
          var selectedOptions = filters[key].data.filter(
            (option) => option.checked
          );
          ...
          if (
            ...
            (selection.value.from != undefined ||
              selection.value.to != undefined)
          ) {
            selectedFilters.push(selection);
          }
          ...
          if (this.fromamount != undefined && this.toamount != undefined) {
            ...
            var displayedAmount = "";
            isValidAmount = this.validateAmount(
        ...
        if (
          ...
          (filters[key].type == "text" && isValidAmount)
        ) {
      ...
      if (isValidAmount) {
        ...
        if (flagEvent == false) {
          ...
          const compEvent = new CustomEvent("fireadvancedfilter", {
            detail: { cmpParams }
          });
          ...
          if (this.applyDate) {
            if (this.fromdate != null) {
              ...
              var labelfrom = this.template
                .querySelector('[data-id="dateFromInput"]')
                .value.substring(0, 10);
              var labelto = this.template
                .querySelector('[data-id="dateToInput"]')
                .value.substring(0, 10);
  ...
  getISOStringFromDateString(dateString, selectedFilters) {
    getCurrentUserTimezoneOffSetInMiliseconds({
      ...
      .catch((error) => {
        console.log(
          "### lwc_account_transactionsModalFormFilter ### getISOStringFromDateString ::: Catch error: " +
            error
        );
      });
  }
