...
export default class Lwc_accountsCard extends LightningElement {
  ...
  get expandirClass() {
    return this.islastupdate == true
      ? "iElement accountsCardLU icon expand slds-show"
      : "iElement accountsCardEOD icon expand slds-show";
  }

  get colapseClass() {
    return this.islastupdate == true
      ? "iElement accountsCardLU icon collapse slds-hide"
      : "iElement accountsCardEOD icon collapse slds-hide";
  }
  ...
  setSvgCountry(iregister) {
    if (this._iregister.key) {
      this.imgCardInfoCountryCode =
        imageFlag + "/" + this._iregister.key + ".svg";
  ...
  sumBalanceExperto() {
    if (
      this.icurrencylist != undefined &&
      this._icurrency != undefined &&
      ...
      getSumBalanceExperto({
        ...
        .catch((error) => {
          if (error) {
            console.log(
              "AccountsCard getSumBalanceExperto Error message: " + error
            );
          } else {
  ...
  getAccountsPerCurrency() {
    ...
    for (var i = 0; i < iReceivedAccounts.length; i++) {
      if (
        !iCurrencies.includes(iReceivedAccounts[i].currencyCodeAvailableBalance)
      ) {
    ...
    for (var j in iCurrencies) {
      ...
      for (var z = 0; z < iReceivedAccounts.length; z++) {
        if (
          iCurrencies[j] == iReceivedAccounts[z].currencyCodeAvailableBalance
        ) {
          lst.push(iReceivedAccounts[z]);
        }
      }
  ...
  showAction() {
    ...
    if (whichKey != undefined) {
      ...
      var rowsSe = this.template
        .querySelector("#" + iOptionsId)
        .querySelectorAll(".contentAccount");
  ...
  displayAmountOne() {
    var bookbalancepagina = this.template.querySelectorAll(
      '[data-id="bookBalance"]'
    );
  ...
  displayAmountTwo() {
    var availableBalancepagina = this.template.querySelectorAll(
  ...
  handleExpand() {
    this.showHideAll();
    this.template
      .querySelectorAll("c-lwc_accounts-card-row-child")
      .forEach((child) => {
        child.handleExpand();
      });
  }
