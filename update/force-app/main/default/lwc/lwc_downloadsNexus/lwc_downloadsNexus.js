...
export default class Lwc_downloadsNexus extends LightningElement {
  ...
  doInit() {
    ...
    this.fileFormatList = [
      ...
      "N43",
      "BAI2",
      this.label.pdfBalances,
      this.label.pdfStatement,
      this.label.excelBalances,
      this.label.excelTransaction /*, 'SIC Contingency'*/
    ];
  ...
  populateComponentData(res) {
    ...
    if (
      window.localStorage.getItem(userId + "_balanceEODGP") != undefined &&
      response != undefined
    ) {
      ...
      this.template
        .querySelector("c-lwc_service-component")
        .saveToCache({ key: "balanceEODGP", data: response_Aux });
  ...
  cleanData() {
    ...
    var dropdownAccnts = this.template.querySelector(
      '[data-id]="dropdownAccounts"]'
    );
    dropdownAccnts = Array.isArray(dropdownAccnts)
      ? dropdownAccnts[0].updateSelection(this.selectedAccounts)
      : dropdownAccnts.updateSelection(this.selectedAccounts);
    //var dropdownCountry = component.find("dropdownCountries");
    var dropdownAccnts = this.template.querySelector(
      '[data-id]="dropdownCountries"]'
    );
    dropdownCountry = Array.isArray(dropdownCountry)
      ? dropdownCountry[0].updateSelection(this._selectedcountries)
      : dropdownCountry.updateSelection(this._selectedcountries);
      ...
      for (var i = 0; i < accList.length; i++) {
        for (var j = 0; j < selectedList.length; j++) {
          if (accList[i] == selectedList[j]) {
            ...
            this.template
              .querySelector('[data-id="dropdownAccounts"]')
              .keepSelection(accList[i]);
          }
        }
      }
    }
  ...
  downloadFiles() {
    ...
    if (
      fileFormat != this.label.excelTransaction &&
      fileFormat != this.label.pdfStatement &&
      fileFormat != this.label.pdfBalances &&
      fileFormat != this.label.excelBalances
    ) {
      params = paramsPost;
    }
