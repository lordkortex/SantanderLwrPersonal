...
export default class Lwc_iptParent extends LightningElement {
    ...
    connectedCallback() {
        ...
        if (urlParams[0] === 'params') {
            ...
            this.getURLParams(urlParams).then(result => {
                ...
                if(this.template.querySelector('c-lwc_ipt-filters') != undefined && this.template.querySelector('c-lwc_ipt-filters') != null){
                    this.template.querySelector('c-lwc_ipt-filters').getAccounts();
                }
                console.log(5);
            });
            console.log(4);
        ...
        try {
            getCurrencies().then((result) => {
                if (result) {
                    ...
                    console.log("ALV this.currencyList[0]: "+ JSON.stringify(this.currencyList[0]));
        ...
        if(sURLVariables[0] != ''){
            for ( var i = 0; i < sURLVariables.length; i++ ) {
                ...
                if (sParameterName[0] === 'c__subsidiaryName') { 
                    ...
                    sParameterName[1] == "true" ? this.fromdetail = true : this.fromdetail = false;
                } else if (sParameterName[0] === 'c__allAccounts') { 
                    sParameterName[1] === undefined ? 'Not found' : this.fullaccountlist = sParameterName[1];	
                } 
            ...
            if(this.accountObj.iSource === 'fromAccount'){
                ...
                accountsTemp.push({label:iAccount.accountNumber,value:iAccount.accountNumber});
             ...
             // 11/03/2021 // modificado para que funcione en One Trade el ir de accounts al tracker
             if(this.fromdetail  == true && this.fullaccountlist != '') {
                            
                var parsedAccounts = JSON.parse(this.fullaccountlist);
                this.accountList = parsedAccounts;
                this.loading = false;
            } 
            else{
                this.loading = false;
            ...
            //END 11/03/2021
            // desalinemaiento 10-03-2021
            this.showAccountPayment = newshowAccountPayment;
        } else {
            this.showAccountPayment = false;
        }
    }
            ...
            getIsCashNexus().then(result => {
                ...
                if(this.isCashNexus == true) {
                    ...
                    this.getCachedAccounts().then(result => {
                        ...
                        console.log(16);
                        this.template.querySelector('c-lwc_ipt-filters').setDataNexus(this.accountList);
                    });
                } else {
    ...
    getPayments(accounts){
        ...
        var filters = '{"searchData":{"latestPaymentsFlag":"NO","_limit":"1000","_offset":"0","inOutIndicator":"OUT","originatorAccountList":[';
        ...
        for(var i=0; i < accList.length; i++) {
            if(accList[i].hasSwiftPayments == 'YES') {
                filters += '{"bankId":'+JSON.stringify(accList[i].codigoBic) + ',"account":{"idType":' + JSON.stringify(accList[i].idType) + ',"accountId":' + JSON.stringify(accList[i].displayNumber) + '}}';
                accListFilters.push({account : accList[i].displayNumber, alias : accList[i].alias, bic : accList[i].codigoBic, id_type : accList[i].idType});
            }
        }
            ...
            getAccounts.then(result => {
                ...
                if (result) {
                    ...
                    for(var i in resJSON){
                        ...
                        accountsTemp.push({label:resJSON[i].accountIdList[0].accountId,value:resJSON[i].accountIdList[0].accountId});
                    }
                    this.accountList = accounts;
                    this.ready = true;
                    this.errorAccount = false;
    ...
    setAccountsAndPayments(response){
        try {  
            ...
            console.log('### lwc_iptParent ### setAccountsAndPayments(response) ::: Catch: ' + e);
        }
    }
}