...
export default class Lwc_iptFilters extends LightningElement {
  ...
  @api getAccounts() {
    try {
      ...
      filter +=
        filterAccount +
        ',"latestPaymentsFlag": "YES","_offset": "0","_limit": "1000", "inOutIndicator": "OUT"}}';
        ...
        if (filters.searchData.inOutIndicator == "OUT") {
          if (filters.searchData.hasOwnProperty("beneficiaryAccountList")) {
            var beneficiaryAccount =
              filters.searchData.beneficiaryAccountList[0].account.accountId;
          ...
          if (filters.searchData.hasOwnProperty("originatorAccountList")) {
            var beneficiaryAccount =
              filters.searchData.originatorAccountList[0].account.accountId;
            this.beneficiaryAccount = beneficiaryAccount;
            count += 1;
          }
        }
      ...
      if (filterAux != "") {
        if (filterAux == this.Label.currencyLabel) {
          ...
          if (JSON.stringify(this.settledFrom) != JSON.stringify(valueAux)) {
            ...
            this.template
              .querySelector("c-lwc_ipt-advanced-filters")
              .setSettledFromOnlyComponent(valueAux);
          }
        } else if (filterAux == "settledTo") {
          if (JSON.stringify(this.settledTo) != JSON.stringify(valueAux)) {
            ...
            this.template
              .querySelector("c-lwc_ipt-advanced-filters")
              .setSettledToOnlyComponent(valueAux);
          }
        }
      }
    } catch (e) {
