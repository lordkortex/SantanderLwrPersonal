...
export default class Lwc_ipt_detailProgressCard extends LightningElement {
  ...
  get mainclassattribute() {
    return this._item.isUnique === true || this._item.isLast === true
      ? this.mainclass + " none"
      : this.mainclass;
  }
  ...
  get itemCountryName() {
    return this._item && this._item.data.countryName
      ? this._item.data.countryName
  ...
  get itemDurationTime() {
    return this._item && this._item.data.durationTime
      ? this._item.data.durationTime
      : "";
  }
  ...
  get itemDeparture() {
    return this._item && this._item.data.departure
      ? this._item.data.departure
      : "";
  }
  get itemSourceCurrency() {
    ...
    try {
      return this._item.data.foreignExchangeDetails.sourceCurrency
        ? this._item.data.foreignExchangeDetails.sourceCurrency
        : "";
    } catch (e) {
      return "";
    }
  ...
  get itemTargetCurrency() {
    ...
    try {
      return this._item.data.foreignExchangeDetails.targetCurrency
        ? this._item.data.foreignExchangeDetails.targetCurrency
        : "";
    } catch (e) {
      return "";
    }
  ...
  get itemExchangeRate() {
    ...
    try {
      return this._item.data.foreignExchangeDetails.exchangeRate
        ? this._item.data.foreignExchangeDetails.exchangeRate
        : "";
    } catch (e) {
      return "";
    }
  ...
  get itemStepFeeCurrency() {
    return this._item && this._item.data.stepFeeCurrency
      ? this._item.data.stepFeeCurrency
      : "";
  }
  get colBankCss() {
  ...
  get isArrivalTraceable() {
    ...
    if (this._item && this._item.data.arrival == this.label.NOT_TRACEABLE)
      arrivalTraceable = false;
    return arrivalTraceable;
  }

  get isDepartureTraceable() {
    let departureTraceable = true;
    if (this._item && this._item.data.departure == this.label.NOT_TRACEABLE)
      departureTraceable = false;
  ...
  updateColClassCss(fxLabel) {
    (this.classCss.colBank = "slds-media__body colBank" + fxLabel),
      ...
      (this.classCss.colArrivalTime =
        "slds-media__body colArrivalTime" + fxLabel),
      (this.classCss.colDuration = "slds-media__body colDuration" + fxLabel),
      (this.classCss.colDepartureTime =
        "slds-media__body colDepartureTime" + fxLabel);
  }
  updateCardClass(cardClass) {
    this.classCss.cardClass =
  ...
  updateTextBoldClass(textbold) {
    this.classCss.textBold =
      "slds-card__header-title slds-truncate " + textbold;
  }
  updateTextRegularClass(textRegular) {
    this.classCss.textRegular =
      "slds-card__header-title slds-truncate " + textRegular;
  }
  setSvgCountry(item) {
    var country = "Default";
  ...
  getDateDifference(arrival, departure) {
    try {
      if (
        arrival !== null &&
        departure !== null &&
        arrival !== this.label.NOT_TRACEABLE
      ) {
        diffDates({ arrival: arrival, departure: departure })
          .then((res) => {
            var firstLine = "";
            var secondLine = "";
            firstLine += res[2] + " m ";
