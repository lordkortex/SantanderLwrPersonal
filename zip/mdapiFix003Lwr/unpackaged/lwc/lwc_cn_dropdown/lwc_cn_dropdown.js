...
export default class Lwc_cn_dropdown extends LightningElement {
  ...
  dinamycClasses = {
    colMargin: { true: "slds-col", false: "slds-col disableMargin" },
    dropdownDisabled: {
      true: "slds-combobox slds-dropdown-trigger disabled",
      false: "slds-combobox slds-dropdown-trigger"
    },
    multiDropdownOrder: {
      true:
        "slds-dropdown slds-dropdown_left slds-dropdown_small dropdownOrder__none",
      false:
        "slds-dropdown slds-dropdown_left slds-dropdown_small dropdownOrder"
    },
    dropdownOrder: {
      true: "slds-dropdown slds-dropdown_left dropdownOrder__none",
      false: "slds-dropdown slds-dropdown_left dropdownOrder"
    }
  };
  ...
  get isSelectedValueNoOne() {
    return (
      this.selectedvalue != null && this.selectedvalue !== this.label.SELECT_ONE
    );
  }
  ...
  @api doInit() {
    if (
      this.issimpledropdown != undefined &&
      this._values &&
      this._values.length > 0
    ) {
      let baseClass = this.issimpledropdown
        ? "slds-dropdown__item"
        : "button-selected icon-check";
      if (
        !this.issimpledropdown &&
        this.selectallvalues != null &&
        this._values != null
      ) {
      ...
      this.listValues = [
        ...this._values.map((value, index) => {
          return {
            ...
            class:
              baseClass +
              (this.selectedvalue === value ? " slds-is-selected" : ""),
        ...
        if (
          ...
          if (selectedValuesList.indexOf(this.selectallvalues) != -1) {
            ...
            selectedValuesList.splice(
              selectedValuesList.indexOf(this.selectallvalues),
              1
            );
          }
