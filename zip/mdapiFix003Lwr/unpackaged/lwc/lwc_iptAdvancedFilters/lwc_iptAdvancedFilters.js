...
export default class Lwc_iptAdvancedFilters extends LightningElement {
  ...
  statusdropdownvalueselectedhandler(event) {
    try {
      ...
      console.log(
        "statusdropdownvalueselectedhandler executed: " +
  ...
  countriesdropdownvalueselectedhandler(event) {
    ...
    console.log(
      "countriesdropdownvalueselectedhandler executed: " +
        event.detail.selectedvalues
    );
  }
  ...
  accountdropdownvalueselectedhandler(event) {
    ...
    console.log(
      "accountdropdownvalueselectedhandler executed: " +
        event.detail.selectedvalues
    );
  }
  ...
  currencydropdownvalueselectedhandler(event) {
    ...
    console.log(
      "currencydropdownvalueselectedhandler executed: " +
        event.detail.selectedvalues
    );
  }
  ...
  get accountLabel1() {
    return this.inoutindicator == "OUT"
      ? this.label.originAccount
      : this.label.beneficiaryAccountLabel;
  }

  get accountLabel2() {
    return this.inoutindicator == "IN"
      ? this.label.originAccount
      : this.label.beneficiaryAccountLabel;
  }
  ...
  connectedCallback() {
    ...
    for (var i in this.accountlist) {
      accountsToDisplayT.push({
        label: this.accountlist[i].account,
        value: this.accountlist[i].account
      });
    }
  ...
  updateFilterData(data) {
    if (data) {
      ...
      if (filter != "") {
        if (
          filter === this.label.Account &&
          JSON.stringify(this.selectedaccount) != JSON.stringify(value)
        ) {
          this.selectedaccount = value;
        } else if (
          filter === this.label.currency &&
          JSON.stringify(this.selectedcurrency) != JSON.stringify(value)
        ) {
          if (value === "") {
          ...
          filter === "settledFrom" &&
          JSON.stringify(this.settledfrom) != JSON.stringify(value)
        ) {
          this.settledfrom = value;
        } else if (
          filter === "settledTo" &&
          JSON.stringify(this.settledto) != JSON.stringify(value)
        ) {
      ...
      if (this.filters != null && this.filters != undefined) {
        const getfilterevent = new CustomEvent("getfilter", {
          detail: {
            ...
            count: this.count,
            noAccountSelected: this.noAccountSelected
          }
        });
  ...
  searchAccountBeneficiary(evt) {
    ...
    if (
      this.timeoutSearchAccount != undefined &&
      this.timeoutSearchAccount != null
    ) {
  ...
  refreshPills() {
    try {
      ...
      var filter =
        '{"searchData": {"latestPaymentsFlag": "NO","_limit": "1000","_offset": "0", "inOutIndicator" : "' +
        this.inoutindicator +
        '",';
      var count = 0;
      ...
      if (
        datefrom != null &&
        datefrom != "" &&
        datefrom != this.label.from &&
        (dateto == null || dateto == "" || dateto == this.label.to)
      ) {
        var today = new Date();
      ...
      if (
        ...
        (datefrom == null || datefrom == "" || datefrom == this.label.from)
      ) {
      ...
      if (
        ...
        filter +=
          '"bankId":"",  "account":{"idType":"", "accountId" :"' +
          beneficiaryaccount +
          '"}}],';
      ...
      if (
        ...
        currency != null &&
        currency != "" &&
        currency != this.label.singleChoice
      ) {
      ...
      if (
        ...
        country != null &&
        country != "" &&
        country != this.label.singleChoice
      ) {
      ...
      if (
        ...
        if (this.fromDetail == this.isDetail) {
          ...
          for (var i in selectedstatus) {
            ...
            if (
              selectedstatus[i] == this.label.payment_statusThree ||
              selectedstatus[i] == this.label.payment_statusFour ||
              selectedstatus[i] == this.label.payment_statusTwo
            ) {
              for (var j in data) {
                filter +=
                  '{"status":"' +
      ...
      if (
        ...
        account != null &&
        account != "" &&
        account != this.label.singleChoice &&
        account.length > 0
      ) {
        if (this.inoutindicator == "OUT") {
          filter += '"originatorAccountList":[';
        ...
        if (
          this.inoutindicator == "OUT" &&
          !filter.includes("amountFrom") &&
          !filter.includes("amountTo") &&
          !filter.includes("valueDateFrom") &&
          !filter.includes("valueDateTo") &&
          !filter.includes("searchText") &&
          !filter.includes("currency") &&
          !filter.includes("beneficiaryCountry") &&
          !filter.includes("paymentStatusList") &&
          !filter.includes("beneficiaryAccountList")
        ) {
          filter = filter.replace("NO", "YES");
        }
        if (
          this.inoutindicator == "IN" &&
          !filter.includes("amountFrom") &&
          !filter.includes("amountTo") &&
          !filter.includes("valueDateFrom") &&
          !filter.includes("valueDateTo") &&
          !filter.includes("searchText") &&
          !filter.includes("currency") &&
          !filter.includes("originatorCountry") &&
          !filter.includes("paymentStatusList") &&
          !filter.includes("originatorAccountList")
        ) {
          filter = filter.replace("NO", "YES");
        }
      }
  ...
  findStatus(status) {
    try {
      ...
      if (status == this.label.payment_statusTwo) {
        ...
        return [
          { status: "ACSP", reason: "G002" },
          { status: "ACSP", reason: "G003" },
          { status: "ACSP", reason: "G004" }
        ];
        //In progress
      } else if (status == this.label.payment_statusThree) {
        return [
          { status: "ACSP", reason: "G000" },
          { status: "ACSP", reason: "G001" }
        ];
      }
    } catch (e) {
      console.log(e);
  ...
  clearStatus() {
    try {
      ...
      if (selectedstatus != undefined && selectedstatus != null) {
        ...
        this.template
          .querySelector("[data-name='statusDropdown']")
          .onSelectionUpdate([this.selectedstatuslabel]);
      }
    } catch (e) {
      console.log(e);
    }
  ...
  clearCurrency() {
    try {
      if (this.selectedcurrency != this.label.singleChoice) {
        this.selectedcurrency = this.label.singleChoice;
        this.template
          .querySelector("[data-name='currencyDropdown']")
          .onSelectionUpdate([this.selectedcurrency]);
      }
    } catch (e) {
      console.log(e);
    }
  ...
  clearAccount() {
    try {
      ...
      if (selectedstatus != undefined && selectedstatus != null) {
        ...
        this.template
          .querySelector("[data-name='accountDropdown']")
          .onSelectionUpdate([this.selectedaccountlabel]);
      }
    } catch (e) {
      console.log(e);
    }
    ...
    try {
      ...
      if (
        ...
        if (parseInt(settledTo) < parseInt(settledFrom)) {
          this.classSettleFrom =
            "slds-input inputShadow lwc_input " + "inputShadowError";
          this.classSettleTo =
            "slds-input inputShadow lwc_input " + "inputShadowError";
  ...
  buildStatus(stringList) {
    ...
    if (stringList != undefined) {
      if (
        stringList.includes(
          '{"status":"ACSC","reason":""},{"status":"ACCC","reason":""}'
        ) === true
      ) {
      ...
      if (
        stringList.includes(
          '{"status":"ACSP","reason":"G002"},{"status":"ACSP","reason":"G003"},{"status":"ACSP","reason":"G004"}'
        ) == true
      ) {
      ...
      if (
        stringList.includes(
          '{"status":"ACSP","reason":"G000"},{"status":"ACSP","reason":"G001"}'
        ) == true
      ) {
