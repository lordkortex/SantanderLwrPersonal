({
  ...
  doInitHelper: function (component, event, helper) {
    ...
    if (isSingleTabSelected == true) {
      Promise.all([helper.getURLParams(component, event, helper)])
        .then(
        ...
        .then(
          $A.getCallback(function (value) {
            return helper.getRawAccountsFiltered(
              component,
              helper,
        ...
        .finally(
          $A.getCallback(function () {
            document.querySelector(
              ".comm-page-custom-landing-payments"
            ).style.overflow = "auto";
              ...
              if (
                ...
                if (!$A.util.isEmpty(result.value)) {
                  if (!$A.util.isEmpty(result.value.output)) {
                    if (
                      !$A.util.isEmpty(result.value.output.paymentStatusList)
                    ) {
                      component.set(
                        "v.singlePaymentStatusBoxes",
                        result.value.output.paymentStatusList
                      );
                    }
                    if (
                      !$A.util.isEmpty(result.value.output.totalNumberOfRecords)
                    ) {
                      component.set(
                        "v.singleNumRecords",
                        result.value.output.totalNumberOfRecords
                      );
                    }
                    resolve("ok");
                  } else {
  ...
  /*
    ...
    24/07/2020		Shahad Naji			Simplify code that handles the success or failure of asynchronous calls.
    */
  getPaymentsInformation: function (
    component,
    event,
    helper,
    isSingleTabSelected
              ...
              if (result != null && result != undefined) {
                if (result.success) {
                  if (!$A.util.isEmpty(result.value)) {
                    if (!$A.util.isEmpty(result.value.output)) {
                      component.set(
                        "v.singleStatusDropdownList",
                        result.value.output.statusList
                      );
                      component.set(
                        "v.singleCurrencyDropdownList",
                        result.value.output.currencyList
                      );
                      component.set(
                        "v.singlePaymentMethodDropdownList",
                        result.value.output.paymentTypeList
                      );
                      component.set(
                        "v.singleCountryDropdownList",
                        result.value.output.countryList
                      );
                      component.set(
                        "v.simpleCountryDropdownList",
                        result.value.output.countryList
                      );
                      component.set(
                        "v.initialSinglePaymentList",
                        result.value.output.paymentsList
                      );
                      component.set(
                        "v.singlePaymentList",
                        result.value.output.paymentsList
                      ...
                      component
                        .find("paymentsLandingTable")
                        .setComponent(result.value.output.paymentsList);
                      component.set("v.isSingleDataLoaded", true);
                      component.set(
                        "v.availableStatuses",
                        result.value.output.availableStatuses
                      );
                      resolve("ok");
                    } else {
                      reject({
                ...
                if (!$A.util.isEmpty(result.value.output)) {
                  if (!$A.util.isEmpty(result.value.output.documentId)) {
                    ...
                    helper.showSuccessToast(
                      ...
                      event,
                      helper,
                      $A.get("$Label.c.PAY_downloadSuccessful"),
                      toastText
                    );
                    resolve(documentId);
                  } else {
    ...
    try {
      ...
      if (sURLVariablesMain[0] == "params") {
              ...
              for (var i = 0; i < sURLVariables.length; i++) {
                  ...
                  if (
                    ...
                    if (sParameterName[1] == "last") {
                      ...
                      window.localStorage.removeItem(
                        userId + "_balanceGP_timestamp"
                      );
                      window.localStorage.removeItem(
                        userId + "_balanceTimestampGP"
                      );
                    ...
                    helper.showSuccessToast(
                      ...
                      $A.get("$Label.c.authorizeSuccess")
                    );
                  }
                }
                ...
                if (sParameterName[0] === "c__saveForLater") {
                  if (
                    ...
                    helper.showSuccessToast(
                      ...
                      event,
                      helper,
                      $A.get("$Label.c.PAY_savedSuccess"),
                      null
                    );
                  }
                }
                ...
                if (sParameterName[0] === "c__reject") {
                  if (
                    ...
                    helper.showSuccessToast(
                      ...
                      helper,
                      $A.get("$Label.c.success"),
                      $A.get("$Label.c.PAY_sendRejected")
                    );
                  }
                }
                if (sParameterName[0] === "c__FFCError") {
                  if (
                    !$A.util.isEmpty(sParameterName[1]) &&
                    ...
                    helper.showToast(
                      ...
                      event,
                      helper,
                      $A.get("$Label.c.FCCError"),
                      $A.get("$Label.c.FCCErrorDescription"),
                      true
                    );
                  }
                }
                if (sParameterName[0] === "c__error") {
                  ...
                  helper.showToast(
                    ...
                    helper,
                    null,
                    $A.get("$Label.c.PAY_ServiceTemporalyDown"),
                    true
                  );
                  //  }
                }
        ...
        if (
          ...
          if (
            sURLVariablesMain[1] === "c__pendingByMe" ||
            sURLVariablesMain[1] === "c__pendingByOthers" ||
            sURLVariablesMain[1] === "c__inReview" ||
            sURLVariablesMain[1] === "c__scheduled" ||
            sURLVariablesMain[1] === "c__completed" ||
            sURLVariablesMain[1] === "c__rejected"
          ) {
  ...
  goToPaymentDetail: function (component, event, helper) {
    ...
    var url =
      "c__currentUser=" +
      JSON.stringify(component.get("v.currentUser")) +
      "&c__paymentID=" +
      paymentID;
        ...
        try {
          ...
          if (!$A.util.isEmpty(urlFilter)) {
            ...
            if (!$A.util.isEmpty(statusName)) {
              ...
              for (
                let i = 0;
                i < paymentStatusBoxes.length && paymentStatusBox == null;
                i++
              ) {
                if (
                  paymentStatusBoxes[i].parsedStatusDescription == statusName
                ) {
