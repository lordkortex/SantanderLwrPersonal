({
  ...
  handleStatusHistory: function (component, event, helper) {
    ...
    for (let i = 0; i < historyLength; i++) {
      ...
      statusItem.time = $A.localizationService.formatTime(
      ...
      if (statusItem.status == "001" && statusItem.reason == "000") {
        ...
        console.log(
          "Status history item not recognized: ",
    ...
    if (!$A.util.isEmpty(authorizationList)) {
      ...
      component.set(
        "v.authorizationTopLine",
        authorizationList[lastAuthorizationElement]
      );
    }
  },
  ...
  handleInReviewModal: function (component, event, helper) {
    ...
    if (
      currentUserGlobalId != paymentUserGlobalId &&
