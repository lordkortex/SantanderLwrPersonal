({
  ...
  handleDoInit: function (component, event, helper) {
    ...
    if (sPageURLMain != "" && sPageURLMain.includes("params")) {
      component
        .find("Service")
        .dataDecryption(component, helper, sPageURLMain, this.handleParams);
      ...
      component.set("v.urlParamsChecked", true);
    } else {
      ...
      for (var i = 0; i < response.length; i++) {
        ...
        switch (sParameterName[0]) {
          ...
          case "c__uetr":
            sParameterName[1] === undefined
              ? "Not found"
              : component.set("v.searchValue", sParameterName[1]);
  ...
  changeCommunityLanguage: function (component, helper, url) {
    ...
    if (
      window.location.search.includes("uetr") &&
      ...
      if (navigatorLanguage === "es") {
        window.location.search != searchHeader + "es"
          ? (window.location.search = searchHeader + "es")
          : "";
      } else if (navigatorLanguage === "pt") {
        window.location.search != searchHeader + "pt_BR"
          ? (window.location.search = searchHeader + "pt_BR")
          : "";
      } else if (navigatorLanguage === "pl") {
        window.location.search != searchHeader + "pl"
          ? (window.location.search = searchHeader + "pl")
          : "";
      } else {
        window.location.search != searchHeader + "en_US"
          ? (window.location.search = searchHeader + "en_US")
          : "";
      }
    }
