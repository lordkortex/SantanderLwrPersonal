({
  ...
  isOneTrade: function (component, helper, response) {
    try {
      ...
      let hasGlobalPositionAccess =
        window.localStorage.getItem(userId + "_hasGlobalPositionAccess") !=
          false &&
        window.localStorage.getItem(userId + "_hasGlobalPositionAccess") !=
          "false"
          ? true
  ...
  handleDoInit: function (component, event, helper) {
    ...
    if (
      ...
      new Date() - new Date(Date.parse(balanceTimestampGP)) <
        parseInt($A.get("$Label.c.refreshBalanceCollout")) * 60000
    ) {
      ...
      component
        ...
        .callApex2(
          ...
          helper,
          "c.retrieveIAMData",
          { isEndOfDay: false, globalId: userId },
          helper.handleInitialData
        );
    }
  },
    ...
    if (!response.isOneTrade) {
      component
        ...
        .callApex2(
          ...
          {
            isEndOfDay: false,
            globalId: $A.get("$SObjectType.CurrentUser.Id")
          },
          helper.handleInitialData
        );
    } else {
      if (response != null && response.is403Error) {
        ...
        if (response != null) {
          ...
          if (
            response.currenciesList != undefined &&
            response.currenciesList != null &&
            response.currenciesList.length > 0
          ) {
          ...
          window.localStorage.setItem(
            ...
            response.canUserSeePaymentsTracker
          );
          window.localStorage.setItem(
            userId + "_hasGlobalPositionAccess",
            response.canUserSeeGP
          );
          ...
          if (response.canUserSeeGP) {
            ...
            component.set(
              "v.totalBookBalanceMapped",
              response.bookBalanceMapped
            );
            component.set(
              "v.availableBookBalanceMapped",
              response.avaibleBookBalanceMapped
            );
            component.set("v.ExchangeRates", response.tiposDeCambio);
            component.set(
              "v.eRatesLastModifiedDate",
              response.eRatesLastModifiedDate
            );
            component.set(
              "v.eRatesLastModifiedDateMain",
              response.eRatesLastModifiedDateMain
            );
            component.set(
              "v.ExchangeRatesToShow",
              response.tiposDeCambio[response.divisaPrincipal]
            );
            // Set the preferred user date and number
            component.set(
              "v.userPreferredDateFormat",
              response.mapUserFormats.dateFormat != ""
                ? response.mapUserFormats.dateFormat
                : "dd/MM/yyyy"
            );
            component.set(
              "v.userPreferredNumberFormat",
              response.mapUserFormats.numberFormat != ""
                ? response.mapUserFormats.numberFormat
                : "###.###.###,##"
            );
            ...
            if (response.headerLastModifiedDate != null) {
              component.set(
                "v.headerLastModifiedDate",
                response.headerLastModifiedDate
              );
              var aux = response.headerLastModifiedDate.split(" ");
              // component.set("v.lastInfoDate", aux[0]+'T'+aux[1]+'.000Z');
              component.set(
                "v.lastInfoDate",
                response.headerLastModifiedDateMain
              );
              component.set(
                "v.lastInfoHour",
                $A.localizationService.formatTime(
                  response.headerLastModifiedDate,
                  "HH:mm"
                )
              );
            ...
            component.set("v.countryGroupResponse", response.accountsByCountry);
            component.set(
              "v.corporateGroupResponse",
              response.accountsByCorporate
            );
          } else {
            component.set("v.countryGroupResponse", {});
            component.set("v.corporateGroupResponse", {});
  ...
  updateCurrencies: function (component, helper) {
    if (component.get("v.mainCurrency") != undefined) {
      ...
      component.set(
        "v.avaibleBookBalance",
        component.get("v.availableBookBalanceMapped")[
          component.get("v.selectedCurrency")
        ]
      );
      component.set(
        "v.totalBookBalance",
        component.get("v.totalBookBalanceMapped")[
          component.get("v.selectedCurrency")
        ]
      );
      ...
      component.get("v.countryGroupResponse").forEach((element) => {
        element.availableBalance =
          element.avaibleBalanceMapped[component.get("v.selectedCurrency")];
        element.totalBalance =
          element.bookBalanceMapped[component.get("v.selectedCurrency")];
      ...
      component.set(
        "v.ExchangeRatesToShow",
        component.get("v.ExchangeRates")[component.get("v.selectedCurrency")]
      );
    }
  },
  ...
  /*
                ...
                */
  checkAdminAndEnrollment: function (component, event, helper) {
    let currentUser = $A.get("$SObjectType.CurrentUser.Id");

    if (currentUser != undefined && currentUser != null) {
      try {
        var action = component.get("c.checkAdminAndEnrollment");
        action.setParams({ currentUser: currentUser });
        action.setCallback(this, function (response) {
          var state = response.getState();
          if (state === "ERROR") {
            var errors = response.getError();
            ...
            if (result != undefined && result != null && result != "null") {
              component.set("v.adminWithoutEnrollment", result);
            }
          }
        });
        $A.enqueueAction(action);
