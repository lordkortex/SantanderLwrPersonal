({
  ...
  checkLastModified: function (component, helper) {
    return new Promise(
      $A.getCallback(function (resolve, reject) {
        ...
        if (
          steps.lastModifiedStep < steps.shownStep &&
          steps.focusStep == steps.lastModifiedStep
        ) {
  ...
  nextStep: function (component, helper) {
    ...
    if (focusStep < totalSteps) {
      ...
      focusStep++;
      helper
        .processStep(component, shownStep, focusStep, false)
        .then(
          $A.getCallback(function (value) {
            return new Promise(
              $A.getCallback((resolve, reject) => {
                var stepComponent = document.getElementById(
                  "step-" + focusStep
                );
                if (stepComponent != null) {
                  ...
                  setTimeout(function () {
                    stepComponent = document.getElementById(
                      "step-" + focusStep
                    );
                    if (stepComponent != null) {
                      stepComponent.scrollIntoView({ behavior: "smooth" });
  ...
  previousStep: function (component, helper) {
    ...
    if (focusStep > 1) {
      ...
      focusStep -= 1;
      var parametros = helper.processStep(
        component,
        shownStep,
        focusStep,
        editPayment
      ...
      $A.getCallback(function (resolve, reject) {
        ...
        if (focusStep == 1) {
          ...
          return helper
            ...
            .then(
              $A.getCallback(function (value) {
                return helper.handleAccountsToB2BDestination(
                  component,
                  helper,
                  value
                );
  ...
  handleAccountsToB2BDestination: function (component, helper, value) {
    return new Promise(
      $A.getCallback(function (resolve, reject) {
        ...
        let paymentDraft = component.get("v.paymentDraft");
        let accountListDestination = helper.removeAccountFromList(
          value,
          paymentDraft.sourceAccount
        );
  ...
  removeAccountFromList: function (listToCheck, accountToDelete) {
    ...
    if (!$A.util.isEmpty(listToCheck)) {
      for (let i = 0; i < listToCheck.length; i++) {
        if (
          listToCheck[i].displayNumber.localeCompare(
            accountToDelete.displayNumber
          ) != 0
        ) {
  ...
  getURLParams: function (component, helper) {
    return new Promise(
      $A.getCallback(function (resolve, reject) {
        ...
        let sPageURLMain = decodeURIComponent(
          window.location.search.substring(1)
        );
        let sURLVariablesMain = sPageURLMain.split("&")[0].split("=");
        if (
          sURLVariablesMain[0] == "params" &&
          !$A.util.isEmpty(sURLVariablesMain[1])
        ) {
          helper
            .decrypt(component, sURLVariablesMain[1])
            .then(
              $A.getCallback(function (results) {
                ...
                if (!$A.util.isEmpty(results)) {
                  ...
                  for (var i = 0; i < sURLVariables.length; i++) {
                    ...
                    if (
                      sParameterName[0] ===
                      $A.get("$Label.c.PARAM_parsedPaymentMethod")
                    ) {
                      if (!$A.util.isEmpty(sParameterName[1])) {
                        let paymentDraft = component.get("v.paymentDraft");
                ...
                if (
                  !$A.util.isEmpty(paymentDetails) &&
                  $A.util.isEmpty(transferType)
                ) {
                ...
                if (
                  ...
                  transferType ==
                    $A.get("$Label.c.PTT_international_transfer_single") ||
                  transferType == "international_transfer_single"
                ) {
        ...
        if (isEditingProcess == true) {
          ...
          if (focusStep == 1) {
            ...
            if (!$A.util.isEmpty(originList)) {
              for (
                let i = 0;
                i < originList.length && sourceAccount == null;
                i++
              ) {
                if (
                  originList[i].displayNumber ==
                  paymentDetails.sourceAccount.trim()
                ) {
            ...
            if (!$A.util.isEmpty(sourceAccount)) {
              ...
              if (
                !isReuseProcess &&
                !$A.util.isEmpty(paymentDetails.paymentId)
              ) {
            ...
            if (!$A.util.isEmpty(destinationList)) {
              for (
                let i = 0;
                i < destinationList.length && destinationAccount == null;
                i++
              ) {
                if (
                  destinationList[i].displayNumber ==
                  paymentDetails.beneficiaryAccount.trim()
                ) {
            ...
            if (
              !$A.util.isEmpty(paymentDetails.operationAmount) &&
              !$A.util.isEmpty(paymentDetails.operationAmount.amount)
            ) {
              if (
                !$A.util.isEmpty(paymentDetails.operationNominalFxDetails) &&
                !$A.util.isEmpty(
                  paymentDetails.operationNominalFxDetails.customerExchangeRate
                )
              ) {
              ...
              !$A.util.isEmpty(paymentDetails.counterValueOperationAmount) &&
              !$A.util.isEmpty(
                paymentDetails.counterValueOperationAmount.amount
              )
            ) {
          ...
          if (state === "SUCCESS") {
            ...
            if (output.success) {
              if (!$A.util.isEmpty(output.value)) {
                if (
                  returnValue.value.limitsResult.toLowerCase() !=
                  ok.toLowerCase()
                ) {
                  resolve("ok");
                } else {
          ...
          if (actionResult.getState() == "ERROR") {
            ...
            if (errors) {
              ...
              helper.showToast(
                ...
                $A.get("$Label.c.Problem_Signature_Structure"),
                true
              );
            }
            reject({
    ...
    return new Promise(
      $A.getCallback(function (resolve, reject) {
        ...
        if (focusStep == 3) {
          component.set("v.spinner", true);
          helper
            .getLimits(component, event, helper)
            .then(
              $A.getCallback(function (value) {
            ...
            .then(
              $A.getCallback(function (value) {
                return helper.getPaymentSignatureStructure(
                  component,
                  event,
                  helper
                );
    ...
    return new Promise(function (resolve, reject) {
      ...
      action.setCallback(this, function (actionResult) {
        if (actionResult.getState() == "SUCCESS") {
          ...
          if (stateRV.success) {
            if (
              !$A.util.isEmpty(stateRV.value.limitsResult) &&
