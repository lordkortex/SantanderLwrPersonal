({
      ...
      try {
        ...
        if (!$A.util.isEmpty(transferType)) {
          ...
          if (transferType == PTT_international_transfer_single) {
            ...
            component.set(
              "v.calendarPlaceholder",
              $A.get("$Label.c.PAY_ProcessDate")
            );
            resolve("ok");
          } else {
            resolve("ok");
  ...
  setValuePaymentDraft: function (component, event, helper) {
    let paymentDraft = component.get("v.paymentDraft");
    paymentDraft.purpose = $A.util.isEmpty(paymentDraft.purpose)
      ? ""
      : paymentDraft.purpose;
    paymentDraft.reference = $A.util.isEmpty(paymentDraft.reference)
      ? ""
      : paymentDraft.reference;
    paymentDraft.chargeBearer = $A.util.isEmpty(paymentDraft.chargeBearer)
      ? ""
