({
  ...
  setFilters: function (component, event, helper) {
    ...
    var sortCurrencyList = helper.sortList(
      component,
      event,
      helper,
      currencyList
  ...
  setFilterCounter: function (component, event, helper) {
    ...
    if (!$A.util.isEmpty(selectedBox)) {
      if (
        ...
        $A.get("$Label.c.PAY_Status_PendingOne")
      ) {
        component.set("v.filterCounter", 1);
      } else if (
        selectedBox.parsedStatusDescription ==
        $A.get("$Label.c.PAY_Status_PendingTwo")
      ) {
        component.set("v.filterCounter", 1);
      } else if (
        selectedBox.parsedStatusDescription ==
        $A.get("$Label.c.PAY_Status_InReviewOne")
      ) {
        component.set("v.filterCounter", 1);
      } else if (
        selectedBox.parsedStatusDescription ==
        $A.get("$Label.c.PAY_Status_ScheduledOne")
      ) {
        component.set("v.filterCounter", 2);
      } else if (
        selectedBox.parsedStatusDescription ==
        $A.get("$Label.c.PAY_Status_CompletedOne")
      ) {
        component.set("v.filterCounter", 2);
      } else if (
        selectedBox.parsedStatusDescription ==
        $A.get("$Label.c.PAY_Status_RejectedOne")
      ) {
        component.set("v.filterCounter", 2);
      }
    }
      ...
      if (isHeaderOptionSelected) {
        ...
        if (!$A.util.isEmpty(selectedBox)) {
          ...
          if (
            ...
            var iToday =
              currentDate.getFullYear() + "-" + aux_month + "-" + aux_date;
            d.push(iToday);
            var toDate = new Date();
            ...
            var nextWeek =
              toDate.getFullYear() + "-" + aux_month + "-" + aux_date;
            d.push(nextWeek);
        ...
        if (!$A.util.isEmpty(component.get("v.selectedSourceAccount"))) {
          console.log(
            "selected source account: " +
              JSON.stringify(component.get("v.selectedSourceAccount"))
          );
        } else {
  ...
  countFilters: function (component, event, helper) {
    ...
    if (
      (fromDecimal != undefined && fromDecimal != null && fromDecimal != "") ||
      (toDecimal != undefined && toDecimal != null && toDecimal != "")
    ) {
      count = count + 1;
