...
export default class lwc_downloadAccountsLu extends LightningElement {
  ...
  handleData(response) {
    ...
    wsContent.push({
      A: this.createCell("Consolidation Currency", true),
      B: centeredCellB,
    ...
    wsContent.push({
      A: this.createCell("Account", true),
    ...
    for (let [key, value] of mapaDat) {
      if (
        (this.sortselected == this.label.currency &&
          Object.keys(value.childList).length > 0) ||
        this.sortselected != this.label.currency
      ) {
        actualParent = key;
        if (this.sortselected == this.label.Country) {
          actualParent = this.countrymap
            .filter((data) => data.split(" - ")[0] == actualParent)[0]
            .split(" - ")[1];
        }
        ...
        wsContent.push({
          A: this.createCell(actualParent, true),
          B: centeredCellB,
          C: this.createCell("", false),
          D: this.createCell("", false),
          E: this.createCell(
            value.totalBalance != undefined
              ? value.totalBalance.wholeNumber_Formatted
              : 0,
            false
          ),
          F: this.createCell(
            value.availableBalance != undefined
              ? value.availableBalance.wholeNumber_Formatted
              : 0,
            false
          )
        });
        ...
        for (let [key2, value2] of subGrouping) {
          if (this.sortselected == this.label.currency && key2 != null) {
            key2 = this.countrymap
              .filter((data) => data.split(" - ")[0] == key2)[0]
              .split(" - ")[1];
          }

          wsContent.push({
            A: this.createCell("   " + (key2 == "null" ? "N/A" : key2), false),
            B: this.createCell("", false),
            C: this.createCell("", false),
            D: this.createCell("", false),
            E: this.createCell(
              value2.totalBalance != undefined
                ? value2.totalBalance.wholeNumber_Formatted
                : 0,
              false
            ),
            F: this.createCell(
              value2.availableBalance != undefined
                ? value2.availableBalance.wholeNumber_Formatted
                : 0,
              false
            )
          });
          ...
          for (var i = 0; i < value2.accountList.length; i++) {
            ...
            wsContent.push({
              A: this.createCell(
                "      " +
                  singleAcc.displayNumber +
                  " - " +
                  singleAcc.subsidiaryName,
                false
              ),
              B: this.createCell("", false),
              C: this.createCell(
                singleAcc.codigoBic + " - " + singleAcc.bankName,
                false
              ),
              D: this.createCell(fullDate, false),
              E: this.createCell(
                singleAcc.amountMainBalance_Formatted != undefined
                  ? singleAcc.amountMainBalance_Formatted.wholeNumber_Formatted
                  : 0,
                false
              ),
              F: this.createCell(
                singleAcc.amountAvailableBalance_Formatted != undefined
                  ? singleAcc.amountAvailableBalance_Formatted
                      .wholeNumber_Formatted
                  : 0,
                false
              )
            });
    ...
    let ws = XLSX.utils.json_to_sheet(wsContent, {
      header: ["A", "B", "C", "D", "E", "F"],
      skipHeader: true
    });
    ...
    ws["!merges"] = [{ s: { r: 3, c: 0 }, e: { r: 3, c: 5 } } /* B1:B6 */];
