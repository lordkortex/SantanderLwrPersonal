({
  ...
  setPagesNumber: function (component, event, helper) {
    try {
      ...
      if (totalPages != null && totalPages != "" && totalPages != undefined) {
        ...
        var pages = Math.ceil(
          totalPages.length / component.get("v.paymentsPerPage")
        );
        if (pages > 1) {
        ...
        if (
          component.get("v.totalRetrieved") <=
          component.get("v.paymentsPerPage")
        ) {
          component.set("v.retrievedSeen", component.get("v.totalRetrieved"));
        } else if (
          parseInt(component.get("v.currentPage")) !=
          component.get("v.pageList").length
        ) {
          ...
          component.set(
            ...
            component.get("v.totalRetrieved") -
              component.get("v.paymentsPerPage") *
                (parseInt(component.get("v.currentPage")) - 1)
          );
        }
  ...
  buildData: function (component, event, helper) {
    try {
      ...
      //Configure the number of records displayed per page
      if (
        component.get("v.totalRetrieved") <= component.get("v.paymentsPerPage")
      ) {
        component.set("v.retrievedSeen", component.get("v.totalRetrieved"));
      } else if (
        parseInt(component.get("v.currentPage")) !=
      ...
      if (currentPage == "1") {
        ...
        if (
          component.get("v.pageList").length > component.get("v.pagesToShow")
        ) {
          component.set("v.end", component.get("v.pagesToShow"));
        } else {
          component.set("v.end", component.get("v.pageList").length);
        }
        console.log(currentPageInitial);
        console.log(currentPage);
        component
          .find("paginationDropdown")
          .updateSelection([currentPageInitial]);
      }
    } catch (e) {
      console.error(e);
    }
        ...
        if (component.get("v.pagesToShow") != pages) {
          if (parseInt(recId) != pages) {
            if (parseInt(recId) >= pages - component.get("v.pagesToShow") + 1) {
              component.set(
                "v.start",
                pages - component.get("v.pagesToShow") + 1 - 1
              );
              component.set("v.end", pages);
            } else {
            ...
            component.set(
              "v.start",
              parseInt(recId) - component.get("v.pagesToShow")
            );
