...
export default class CmpOTVTableUsersLanding extends LightningElement {
  ...
  get tabClass() {
    return (
      "image " + this.colors[Math.floor(Math.random() * this.colors.length)]
    );
  }
    ...
    if (this.selectedUser != null) {
      calloutGetUsersAccounts({
        ...
        .finally(() => {
          ...
          getLstSubsidiaries({ lstAccounts: JSON.stringify(this.lstAccounts) })
            .then((result) => {
  ...
  closeuserinfo(event) {
    ...
    const saveUserInfoParent = new CustomEvent("saveuserinfo", {
      detail: {
