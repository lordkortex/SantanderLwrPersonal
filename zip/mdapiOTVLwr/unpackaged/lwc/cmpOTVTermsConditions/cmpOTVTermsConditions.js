...
export default class CmpOTVTermsConditions extends NavigationMixin(
  LightningElement
) {
  label = {
    back,
  ...
  goCompletedTerms() {
    ...
    termsConditionsSignature({ pdfName: this.pdfName })
      .then()
      .catch((error) => {
        this.error = error;
      });
    ...
    cometdlib.handshake(function (status) {
      if (status.successful) {
        ...
        getScauId()
          ...
          .finally(() => {
            try {
              var newSubscription = cometdlib.subscribe(
                "/event/OTPValidationOTV__e",
                (platformEvent) => {
                  console.log(
                    "scaUidPlataforma" + platformEvent.data.payload.scaUid__c
                  );
                  ...
                  if (self.payloadStatus) {
                    ...
                    console.log(pdfName);
                    termsConditionsSignature({ pdfName: pdfName })
                      .then()
                      .catch((error) => {
                        self.error = error;
                      });
