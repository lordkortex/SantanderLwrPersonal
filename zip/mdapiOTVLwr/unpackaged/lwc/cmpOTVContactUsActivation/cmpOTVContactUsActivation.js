...
export default class CmpOTVContactUsActivation extends NavigationMixin(
  LightningElement
) {
  label = {
  ...
  fileData;
  uploadedFiles(event) {
    this.uploadedFile = event.target.value.match(
      /[\/\\]([\w\d\s\.\-\(\)]+)$/
    )[1];
  ...
  submitCase() {
    if (this.fileData != undefined) {
      ...
      createCase({
        valueTopic: this.selectedTopic,
        valueIssue: this.selectedIssue,
