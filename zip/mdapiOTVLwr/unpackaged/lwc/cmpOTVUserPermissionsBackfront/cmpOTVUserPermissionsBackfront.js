...
/*import calloutGetUsersAccounts  from '@salesforce/apex/CNT_OTV_UsersLanding.calloutGetUserAccounts';
    ...
    import setUserInfo              from '@salesforce/apex/CNT_OTV_UserInfo.setUserInfo';*/
...
export default class cmpOTVUserPermissionsBackfront extends LightningElement {
  ...
  changeaccount(event) {
    if (this.subsidiarieslistNew.includes(event.detail.account)) {
      this.subsidiarieslistNew = this.subsidiarieslistNew.filter(
        (value) => value !== event.detail.account
      );
    } else {
    ...
    var accountElement = listaAuxiliar.find(
      (account) => account.accountId == event.detail.account
    );
  ...
  saveUserInfo() {
    ...
    for (var i = 0; i < this.subsidiarieslistNew.length; i++) {
      let accountElement = this.lstaccounts.find(
        (account) => account.accountId == this.subsidiarieslistNew[i]
      );
    ...
    if (
      this.lstUpdatedAccounts != undefined &&
      this.lstUpdatedAccounts.length > 0
    ) {
      calloutUpdateUserAccounts({
        accountList: JSON.stringify(this.lstUpdatedAccounts),
        userId: this.userselected.globalId,
    ...
    const saveUserInfo = new CustomEvent("closeuserinfo", {
      detail: {
        isClicked: false,
    ...
    if ((isRenderedCallback && !this.countriesloaded) || !isRenderedCallback) {
      ...
      for (var i = 0; i < this.lstaccounts.length; i++) {
        if (this.lstaccounts[i].status == "ACTIVE") {
          var country = lstcountriesaux.find(
            (cntry) => cntry.value == this.lstaccounts[i].country
          );
  ...
  handleCountryChange(event) {
    ...
    var country = countrylist.find(
      (cntry) => cntry.value == event.target.dataset.item
    );
    ...
    if (!event.target.checked) {
      ...
      for (var i = 0; i < listaAuxiliar.length; i++) {
        if (
          listaAuxiliar[i].country == country.value &&
          listaAuxiliar[i].status == "ACTIVE"
        ) {
          ...
          if (!this.subsidiarieslistNew.includes(listaAuxiliar[i].accountId)) {
            ...
            this.subsidiarieslistNew = this.subsidiarieslistNew.filter(
              (value) => value !== listaAuxiliar[i].accountId
            );
          }
        }
      }
