...
export default class CmpOTVSubsidiariesSelection extends LightningElement {
  ...
  enrollmentAcct(event) {
    try {
      if (
        ...
        "cardBeneficiary slds-card card-selected"
      ) {
        event.currentTarget.className = "cardBeneficiary slds-card";
        this.subsidiariesList = this.subsidiariesList.filter(
          (value) => value !== event.target.getAttribute("data-item")
        );
      } else {
        event.currentTarget.className =
          "cardBeneficiary slds-card card-selected";
  ...
  showSupport(e) {
    ...
    var span = this.template.querySelector(
      ".slds-button primary.quickButton_icon_text"
    );
  ...
  showIcon(e) {
    ...
    var span = this.template.querySelector(
      ".slds-button primary.quickButton_icon"
    );
