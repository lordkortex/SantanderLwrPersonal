...
export default class CmpOTVB2BToast extends LightningElement {
  ...
  test() {
    return (
      "slds-notify_container slds-is-fixed " +
      (this.static ? "toast_static" : "")
    );
  }
  ...
  test2() {
    return (
      ...
      (this.noReload ? " " : " action") +
      " slds-scrollable_y slds-theme_" +
      this.functionTypeClass
    );
  }

  test3() {
    return (
      "slds-icon_container slds-m-right_small slds-no-flex slds-align-top button-selected slds-icon-utility-" +
      this.functionTypeClassIcon +
      " icon-" +
      this.functionTypeClassIcon
    );
  }
  ...
  test4() {
    return (
      "slds-button slds-button_icon icon-" + (this.action ? "action" : "reload")
    );
  }
