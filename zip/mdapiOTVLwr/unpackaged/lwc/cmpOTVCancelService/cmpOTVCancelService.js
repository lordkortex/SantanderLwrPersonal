...
export default class CmpOTVCancelService extends LightningElement {
    ...
    arrayOptions = [{ label: this.label.cmpOTVCancelService_15, value: this.label.cmpOTVCancelService_15},
                    { label: this.label.cmpOTVCancelService_16, value: this.label.cmpOTVCancelService_16 },
                    { label: this.label.cmpOTVCancelService_17, value: this.label.cmpOTVCancelService_17 },
                    { label: this.label.cmpOTVCancelService_18, value: this.label.cmpOTVCancelService_18 }
                   ];
    reason = 'Reason';
    termsAccepted = false;
    title;
    ...
    cancelService(event){
        ...
        this.checkInputs();
        if((this.reasonText != null && this.description != null) && this.termsAccepted ){
            createCancelServiceCase({Reason:this.reasonText, Description:this.description}).finally(() => {
                declineOTVInvitation({reason   : '',
                                      details  : '',
                                      status   : 'Cancelled'})
                .catch(error => {
                    console.log(error);
    ...
    checkInputs(){
        ...
        this.template.querySelectorAll('[data-id =checkbox]').forEach(element => {
            if((this.reasonText != null && this.reasonText != '') && (this.description != null && this.description != '')){
