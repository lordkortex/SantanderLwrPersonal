...
export default class Lwc_loginAsLookup extends LightningElement {
	...
	doInit() {
        if(this.template.querySelector('[data-id="resultsDiv"]')){
            this.template.querySelector('[data-id="resultsDiv"]').classList.toggle('slds-is-open');
        }
		if( this.value ) {
			this.searchRecordsHelper(this.value);
		}
        if(this.template.querySelector('[data-id="listbox-id-1"]')){
            this.template.querySelector('[data-id="listbox-id-1"]').style="max-height:" + (8 + (this.recordCount * 40)) + "px";
        }
	}
	...
	searchRecords() {
        ...
        this.searchString = this.template.querySelector('[data-id="inputLookup"').value;

        if( this.searchString ) {
		    this.searchRecordsHelper('');
        } else {
            this.template.querySelector('[data-id="resultsDiv"]').classList.remove('slds-is-open');
        }
	}
	...
	selectItem(event) {
        if(event.currentTarget.dataset.id) {
    		...
    		var index = recordsList.findIndex(x => x.value === event.currentTarget.dataset.id)
            if(index != -1) {
    ...
    showRecords() {
        if(this.recordsList && this.searchString) {
            this.template.querySelector('[data-id="resultsDiv"]').classList.add('slds-is-open');
        }
	}
