...
export default class Lwc_backFrontGPITrackerSearch extends LightningElement {
  ...
  doInit() {
    ...
    var options = [
      { label: this.label.Emitidos, value: "OUT" },
      { label: this.label.Recibidos, value: "IN" }
    ];
    this.typeValue = "OUT";
    ...
    try {
      ...
      if (selectedStatus != undefined && selectedStatus != null) {
        if (clear.includes("all")) {
          ...
          this.template
            .querySelector('[data-id="statusDropdown"]')
  ...
  removePill(event) {
    try {
      ...
      if (
        ...
        if (
          this.template
            .querySelector('[data-id="GPISearch"]')
            .classList.hasClass("slds-hide") == true
        ) {
  ...
  apply() {
    ...
    if (
      this.datesValidity == true &&
      this.settledErrorTo == "" &&
      this.template
        .querySelector('[data-id="mandatoryFields"]')
        .classList.contains("slds-hide") &&
      this.template
        .querySelector('[data-id="mandatoryBic"]')
        .classList.contains("slds-hide")
    ) {
    ...
    try {
      ...
      if (
        ...
        if (parseInt(settledTo) < parseInt(settledFrom)) {
          ...
          this.template
            .querySelector('[data-id="settledErrorTo"]')
            .classList.remove("slds-hide");
          this.template
            .querySelector('[data-id="settledTo"]')
            .classList.add("inputShadowError");
          this.template
            .querySelector('[data-id="settledErrorFrom"]')
            .classList.remove("slds-hide");
          this.template
            .querySelector('[data-id="settledFrom"]')
            .classList.add("inputShadowError");
          this.settledErrorTo = this.label.toAmountLowerThanFrom;
          error = "error";
        } else {
          ...
          this.template
            .querySelector('[data-id="settledTo"]')
            .classList.remove("inputShadowError");
          this.template
            .querySelector('[data-id="settledErrorFrom"]')
            .classList.add("slds-hide");
          this.template
  ...
  searchUETR() {
    ...
    if (inputValue == undefined || inputValue == null || inputValue == "") {
      ...
      this.template
        .querySelector('[data-id="uetrError"]')
        .classList.add("slds-hide");
      valid = false;
    } else {
  ...
  searchUETRHelper(uetrCode) {
    try {
      var filter =
        '{"searchData": {"latestPaymentsFlag": "NO","_limit":"1000","_offset":"0","paymentId":"' +
        uetrCode +
        '","inOutIndicator":"OUT"}}';
      this.filters = filter;
      ...
      getISO2Values()
        .then((result) => {
          ...
          setTimeout(() => {
            ...
            this.template
              .querySelector('[data-id="headerSection"]')
              .classList.remove("slds-hide");
          }, 2000);
        })
        .catch((error) => {
  ...
  refreshPills() {
    try {
      ...
      var filter =
        '{"searchData": {"latestPaymentsFlag": "NO","_limit":"1000","_offset":"0",';
      ...
      if (
        ...
        beneficiaryAccount !=
          "" /*  && beneficiaryBIC!=null && beneficiaryBIC!=""*/
      ) {
      ...
      if (
        originCountry != null &&
        originCountry != "" &&
        originCountry != this.label.selectOne
      ) {
        filter +=
          '"originatorCountry":"' + originCountry.substring(0, 2) + '",';
      }
  ...
  openSearch() {
    try {
      ...
      if (searchArea != undefined && searchArea != null) {
        ...
        this.template
          .querySelector('[data-id="searchIcon"]')
          .classList.toggle("button-search-open");
      }
    } catch (e) {
      console.log(e);
    }
    ...
    if (
      ...
      if (this._selecteduert == "") {
        ...
        if (
          (this.selectedOrderingAccount == "" ||
            this.selectedOrderingBIC == "") &&
          (this.beneficiaryccount == "" || this.beneficiaryBIC == "")
        ) {
  ...
  validateRanges() {
    var fromValue = this.template.querySelector('[data-id="valueDateFrom"]')
      .value;
    ...
    if (toValue != this.label.to) {
      if (toValue != null) {
        ...
        this.dateErrorTo = this.label.IncorrectDateFormat;
        this.template
          .querySelector('[data-id="valueDateTo"]')
          .setCustomValidity("");
        this.template.querySelector('[data-id="valueDateTo"]').reportValidity();
      }
    }
      ...
      if (fromValue != null) {
        if (fromValue.match("\\d{4}-\\d{2}-\\d{2}") != null) {
          ...
          if (fromValue <= today) {
            ...
            this.template
              .querySelector('[data-id="valueDateFrom"]')
              .setCustomValidity("");
            this.template
              .querySelector('[data-id="valueDateFrom"]')
              .reportValidity();
