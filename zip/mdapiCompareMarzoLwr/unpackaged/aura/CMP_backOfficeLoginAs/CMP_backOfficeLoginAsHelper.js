({
    getUserAccessToken : function(component, helper, response) {
        let params = {
            ...
            targetUserId : component.get("v.selectedUserId")
        };
        component.find("Service").callApex2(component, helper,"c.getUserAccessToken", params, helper.setResponseData);
    },
    ...
    setResponseData : function(component, helper, response) { 
        ...
        {
            let loginUrl = $A.get("$Label.c.domain") + "/secur/frontdoor.jsp?sid="+ response.accessToken;
            redirectUrl = loginUrl +"&retURL=" + $A.get("$Label.c.domain") + "/s/";
            
        }
        else
        {
            let loginUrl = $A.get("$Label.c.domainCashNexus") + "/secur/frontdoor.jsp?sid="+ response.accessToken;
            redirectUrl = loginUrl +"&retURL=" + $A.get("$Label.c.domainCashNexus") + "/s/";
        ...
        component.find("Service").callApex2(component, helper, "c.createAuditRecord", params, undefined);
    }
})